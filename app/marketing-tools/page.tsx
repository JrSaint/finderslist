...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      "Discover the best marketing tools for SEO, email marketing, social media, analytics, CRM, and advertising. Browse 40+ curated marketing tools with honest reviews, pricing breakdowns, and real-world use cases.",
    keywords: [
      "marketing tools",
...
export default async function MarketingToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedMarketingTools();
  const allTools = getAllMarketingTools();
  const categories = getAllMarketingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterMarketingTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Marketing Tools", item: "https://www.finderslist.com/marketing-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    description: "Discover the best marketing tools for SEO, email marketing, social media, analytics, CRM, and advertising. Browse 40+ curated marketing tools with honest reviews, pricing breakdowns, and real-world use cases.",
    numberOfItems: allTools.length,
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/marketing-tools/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_80%_60%,rgba(16,185,129,0.06),transparent)] pointer-events-none" />
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated marketing tools
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <MarketingFilterBar resultCount={filteredTools.length} />
          </Suspense>
        </section>
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/marketing-tools" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">
                  Clear all filters →
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/marketing-tools" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getMarketingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-white group-hover:text-emerald-300 transition-colors leading-tight">
                        {info.label}
                      </span>
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/marketing-tools" />
                ))}
              </div>
            </section>
          </>
