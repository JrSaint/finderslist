...
import type { MarketingCategory } from "@/lib/marketing-tools";
...
export async function generateStaticParams() {
  return getAllMarketingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = MARKETING_CATEGORIES[slug as MarketingCategory];
  if (!category) return {};
  const tools = getMarketingToolsByCategory(slug as MarketingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "marketing software", category.label],
    alternates: { canonical: `https://www.finderslist.com/marketing-tools/category/${slug}` },
  };
}

export default async function MarketingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!MARKETING_CATEGORIES[slug as MarketingCategory]) notFound();

  const category = MARKETING_CATEGORIES[slug as MarketingCategory];
  const tools = getMarketingToolsByCategory(slug as MarketingCategory);
  const allCategories = getAllMarketingCategories();
  const totalTools = getAllMarketingTools().length;
        ...
        <div className="flex-1 min-w-0">
          ...
          {tools.length === 0 ? (
            <div className="text-center py-16 text-slate-500">
              ...
              <Link href="/marketing-tools/submit" className="text-emerald-400 hover:text-emerald-300">
                Submit one!
              </Link>
            </div>
          ) : (
          ...
          <div className="sticky top-24">
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link
                ...
                className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center"
              >
                Submit a Tool
              </Link>
