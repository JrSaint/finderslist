...
import {
  getFeaturedSubscriptionBillingTools,
  getAllSubscriptionBillingTools,
  getAllSubscriptionBillingCategories,
  SUBSCRIPTION_BILLING_CATEGORIES,
  getSubscriptionBillingCategoryCount,
  filterSubscriptionBillingTools,
} from "@/lib/subscription-billing-platforms";
import { SUBSCRIPTION_BILLING_EDITORIAL } from "@/data/subscription-billing-platforms";
...
export default async function SubscriptionBillingPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSubscriptionBillingTools();
  const allTools = getAllSubscriptionBillingTools();
  const categories = getAllSubscriptionBillingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSubscriptionBillingTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Subscription Billing Platforms",
    description: "Compare the best subscription billing and recurring revenue platforms. Reviews of Chargebee, Stripe Billing, Paddle, Recurly, and more.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/subscription-billing-platforms/tools/${tool.slug}`,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated subscription billing platforms
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of subscription billing platforms. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/subscription-billing-platforms" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/subscription-billing-platforms"
        ...
        {!isFiltering && (
          <EditorialSection
            title={SUBSCRIPTION_BILLING_EDITORIAL.title}
            intro={SUBSCRIPTION_BILLING_EDITORIAL.intro}
            buyerGuide={SUBSCRIPTION_BILLING_EDITORIAL.buyerGuide}
            faq={SUBSCRIPTION_BILLING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/subscription-billing-platforms" />
          <ComparisonTable
            tools={featured as never[]}
            categories={SUBSCRIPTION_BILLING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/subscription-billing-platforms"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/subscription-billing-platforms" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/subscription-billing-platforms" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = SUBSCRIPTION_BILLING_CATEGORIES[cat];
                  const count = getSubscriptionBillingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/subscription-billing-platforms/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/subscription-billing-platforms" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="subscription-billing-platforms" />
      </div>
    </div>
  );
}
