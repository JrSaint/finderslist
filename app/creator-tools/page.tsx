...
import {
  getFeaturedCreatorTools,
  getAllCreatorTools,
  getAllCreatorCategories,
  CREATOR_CATEGORIES,
  getCreatorCategoryCount,
  filterCreatorTools,
  CREATOR_ROLES,
...
const roles = Object.fromEntries(
  Object.entries(CREATOR_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function CreatorToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCreatorTools();
  const allTools = getAllCreatorTools();
  const categories = getAllCreatorCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCreatorTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/creator-tools/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/creator-tools" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/creator-tools" className="text-orange-400 hover:text-orange-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/creator-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getCreatorCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/creator-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-orange-500/30 transition-all group">
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/creator-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="creator-tools" />
      </div>
    </div>
  );
}
