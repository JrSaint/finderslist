...
import {
  getProductivityToolBySlug,
  getAllProductivityTools,
  getProductivityToolsByCategory,
  PRODUCTIVITY_CATEGORIES,
  PRODUCTIVITY_PRICING_LABELS,
  PRODUCTIVITY_PRICING_COLORS,
  PRODUCTIVITY_ROLES,
  type ProductivityRoleKey,
...
export async function generateStaticParams() {
  return getAllProductivityTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getProductivityToolBySlug(slug);
  if (!tool) return {};
  const category = PRODUCTIVITY_CATEGORIES[tool.category];
  return {
    ...
    keywords: [tool.name, ...tool.tags, category.label, "productivity tool review", "productivity software 2026"],
    openGraph: {
      images: ["/og-image.png"],
      title: `${tool.name} Review 2026 | FindersList`,
      description: `${tool.tagline}. See honest pros, cons, pricing, and use cases.`,
    },
    alternates: { canonical: `https://www.finderslist.com/productivity-tools/tools/${tool.slug}` },
  };
}

function getBestForRoles(tool: ReturnType<typeof getProductivityToolBySlug>) {
  if (!tool) return [];
  return (Object.entries(PRODUCTIVITY_ROLES) as [ProductivityRoleKey, typeof PRODUCTIVITY_ROLES[ProductivityRoleKey]][])
    .filter(([, config]) =>
...
export default async function ProductivityToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getProductivityToolBySlug(slug);
  ...
  const category = PRODUCTIVITY_CATEGORIES[tool.category];
  const relatedTools = getProductivityToolsByCategory(tool.category)
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Productivity Tools", item: "https://www.finderslist.com/productivity-tools" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/productivity-tools/category/${tool.category}` },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href="/productivity-tools" className="hover:text-slate-300 transition-colors">Productivity Tools</Link>
        <span>/</span>
        <Link href={`/productivity-tools/category/${tool.category}`} className="hover:text-slate-300 transition-colors">
          {category.label}
        </Link>
        <span>/</span>
        ...
        <div className="flex-1 min-w-0">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    {tool.featured && (
                      <span className="text-xs bg-teal-500/30 text-teal-200 border border-teal-400/30 rounded-full px-2.5 py-0.5">
                        Featured
                      </span>
                    )}
                    <span className={`text-xs border rounded-full px-2.5 py-0.5 ${PRODUCTIVITY_PRICING_COLORS[tool.pricing]}`}>
                      {PRODUCTIVITY_PRICING_LABELS[tool.pricing]}
                    </span>
                  </div>
                  <p className="text-lg text-white/80 mb-3 leading-snug">{tool.tagline}</p>
                  <Link
                    href={`/productivity-tools/category/${tool.category}`}
              ...
              <div className="space-y-3">
                {tool.useCases.map((useCase, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-teal-500/20 border border-teal-500/30 flex items-center justify-center flex-shrink-0 text-xs font-bold text-teal-400 mt-0.5">
                      {i + 1}
                    </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-6 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {bestForRoles.map((role) => (
                  <Link
                    key={role.key}
                    href={`/productivity-tools?role=${role.key}`}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/10 bg-slate-800/60 hover:border-teal-500/30 hover:bg-slate-700/60 text-sm text-slate-300 hover:text-white transition-all"
                  >
          ...
          {relatedTools.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTools.map((t) => (
                  <ToolCard key={t.slug} tool={t as never} basePath="/productivity-tools" />
                ))}
              </div>
            </section>
          )}
          ...
          <div className="sticky top-24 space-y-4">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <p className="text-xs text-slate-600 mt-2 text-center">
                {PRODUCTIVITY_PRICING_LABELS[tool.pricing]} · {category.label}
              </p>
            </div>
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <Link
                href={`/productivity-tools/category/${tool.category}`}
                className="block w-full rounded-lg border border-white/10 hover:border-teal-500/30 hover:bg-slate-800 px-4 py-2.5 text-sm text-slate-400 hover:text-white transition-all text-center"
              >
                See all tools →
              </Link>
            </div>
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              <p className="text-sm text-slate-400 mb-3">Know a great productivity tool?</p>
              <Link
                href="/productivity-tools/submit"
