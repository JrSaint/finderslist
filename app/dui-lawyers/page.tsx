...
import { DUI_LAWYER_EDITORIAL } from "@/data/dui-lawyers";
...
export default async function DUILawyersPage({ searchParams }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "DUI Lawyers", item: "https://www.finderslist.com/dui-lawyers" },
    ],
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={DUI_LAWYER_EDITORIAL.title}
            intro={DUI_LAWYER_EDITORIAL.intro}
            buyerGuide={DUI_LAWYER_EDITORIAL.buyerGuide}
            faq={DUI_LAWYER_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/dui-lawyers" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DUI_LAWYER_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/dui-lawyers"
          />
          </>
        )}
        ...
        <RelatedDirectories currentSlug="dui-lawyers" />
      </div>
    </div>
  );
}
