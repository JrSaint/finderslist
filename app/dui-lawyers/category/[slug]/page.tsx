...
export async function generateStaticParams() {
  return getAllDUILawyerCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = DUI_LAWYER_CATEGORIES[slug as DUILawyerCategory];
  if (!category) return {};
...
export default async function DUILawyerCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!DUI_LAWYER_CATEGORIES[slug as DUILawyerCategory]) notFound();

  const category = DUI_LAWYER_CATEGORIES[slug as DUILawyerCategory];
  const tools = getDUILawyerToolsByCategory(slug as DUILawyerCategory);
  const allCategories = getAllDUILawyerCategories();
  const totalTools = getAllDUILawyerTools().length;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `Best ${category.label} Attorneys`,
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="lg:w-72 flex-shrink-0">
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
            ...
            <Link href="/dui-lawyers" className="mt-4 block text-center text-xs text-slate-500 hover:text-red-400 transition-colors">
