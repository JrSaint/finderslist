...
import {
  getFeaturedAIAgentsTools,
  getAllAIAgentsTools,
  getAllAIAgentsCategories,
  AI_AGENTS_CATEGORIES,
  getAIAgentsCategoryCount,
  filterAIAgentsTools,
} from "@/lib/ai-agent-platforms";
import { AI_AGENTS_EDITORIAL } from "@/data/ai-agent-platforms";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/ai-agent-platforms" } };
  }
  return {
    title: "Best AI Agent Platforms (2026) — 20+ Tools Reviewed",
    description: "Compare the best AI agent platforms for building autonomous agents and AI workflows. Reviews of LangChain, Make.com, CrewAI, and more.",
    keywords: ["ai agent platform","best ai agent tools 2026","autonomous ai agents","llm agent framework","ai workflow automation"],
    alternates: { canonical: "https://www.finderslist.com/ai-agent-platforms" },
  };
}

export default async function AIAgentsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedAIAgentsTools();
  const allTools = getAllAIAgentsTools();
  const categories = getAllAIAgentsCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterAIAgentsTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/ai-agent-platforms/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (AI_AGENTS_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated ai agent platforms
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of ai agent platforms. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/ai-agent-platforms" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={AI_AGENTS_EDITORIAL.title}
            intro={AI_AGENTS_EDITORIAL.intro}
            buyerGuide={AI_AGENTS_EDITORIAL.buyerGuide}
            faq={AI_AGENTS_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ai-agent-platforms" />
          <ComparisonTable
            tools={featured as never[]}
            categories={AI_AGENTS_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/ai-agent-platforms"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/ai-agent-platforms" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/ai-agent-platforms" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = AI_AGENTS_CATEGORIES[cat];
                  const count = getAIAgentsCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/ai-agent-platforms/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/ai-agent-platforms" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="ai-agent-platforms" />
      </div>
    </div>
  );
}
