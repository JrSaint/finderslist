...
import type { AIAgentsCategory } from "@/lib/ai-agent-platforms";
...
export async function generateStaticParams() {
  return getAllAIAgentsCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = AI_AGENTS_CATEGORIES[slug as AIAgentsCategory];
  if (!category) return {};
  const tools = getAIAgentsToolsByCategory(slug as AIAgentsCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "ai agent platforms", category.label],
    alternates: { canonical: `https://www.finderslist.com/ai-agent-platforms/category/${slug}` },
  };
}

export default async function AIAgentsCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!AI_AGENTS_CATEGORIES[slug as AIAgentsCategory]) notFound();

  const category = AI_AGENTS_CATEGORIES[slug as AIAgentsCategory];
  const tools = getAIAgentsToolsByCategory(slug as AIAgentsCategory);
  const allCategories = getAllAIAgentsCategories();
  const totalTools = getAllAIAgentsTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/ai-agent-platforms/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/ai-agent-platforms" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
