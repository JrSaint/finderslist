...
import {
  getFeaturedCreditCardTools,
  getAllCreditCardTools,
  getAllCreditCardCategories,
  CREDIT_CARD_CATEGORIES,
  getCreditCardCategoryCount,
  filterCreditCardTools,
} from "@/lib/credit-cards";
import { CREDIT_CARD_EDITORIAL } from "@/data/credit-cards";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/credit-cards" } };
  }
  return {
    title: "Best Credit Cards (2026) — 15+ Cards Reviewed & Compared",
    description: "Compare the best credit cards for cash back, travel rewards, balance transfers, and business. Reviews of Chase, Amex, Capital One, and more.",
    keywords: ["best credit cards","credit card comparison 2026","cash back credit cards","travel rewards cards","business credit cards"],
    alternates: { canonical: "https://www.finderslist.com/credit-cards" },
  };
}

export default async function CreditCardPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCreditCardTools();
  const allTools = getAllCreditCardTools();
  const categories = getAllCreditCardCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCreditCardTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/credit-cards/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (CREDIT_CARD_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 text-sm text-green-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated credit cards
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of credit cards. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/credit-cards" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={CREDIT_CARD_EDITORIAL.title}
            intro={CREDIT_CARD_EDITORIAL.intro}
            buyerGuide={CREDIT_CARD_EDITORIAL.buyerGuide}
            faq={CREDIT_CARD_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/credit-cards" />
          <ComparisonTable
            tools={featured as never[]}
            categories={CREDIT_CARD_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/credit-cards"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/credit-cards" className="text-green-400 hover:text-green-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/credit-cards" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = CREDIT_CARD_CATEGORIES[cat];
                  const count = getCreditCardCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/credit-cards/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/credit-cards" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="credit-cards" />
      </div>
    </div>
  );
}
