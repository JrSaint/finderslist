...
import {
  getFeaturedInvoicingTools,
  getAllInvoicingTools,
  getAllInvoicingCategories,
  INVOICING_CATEGORIES,
  getInvoicingCategoryCount,
  filterInvoicingTools,
} from "@/lib/invoicing-billing-software";
import { INVOICING_EDITORIAL } from "@/data/invoicing-billing-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/invoicing-billing-software" } };
  }
  return {
    title: "Best Invoicing & Billing Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best invoicing and billing software for freelancers, agencies, and SaaS companies. Reviews of FreshBooks, HoneyBook, Stripe, and more.",
    keywords: ["invoicing software","billing software 2026","online invoicing","invoice generator","billing platform"],
    alternates: { canonical: "https://www.finderslist.com/invoicing-billing-software" },
  };
}

export default async function InvoicingPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedInvoicingTools();
  const allTools = getAllInvoicingTools();
  const categories = getAllInvoicingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterInvoicingTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/invoicing-billing-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (INVOICING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-1.5 text-sm text-cyan-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated invoicing & billing software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of invoicing & billing software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/invoicing-billing-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/invoicing-billing-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={INVOICING_EDITORIAL.title}
            intro={INVOICING_EDITORIAL.intro}
            buyerGuide={INVOICING_EDITORIAL.buyerGuide}
            faq={INVOICING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/invoicing-billing-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={INVOICING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/invoicing-billing-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/invoicing-billing-software" className="text-cyan-400 hover:text-cyan-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/invoicing-billing-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = INVOICING_CATEGORIES[cat];
                  const count = getInvoicingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/invoicing-billing-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/invoicing-billing-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="invoicing-billing-software" />
      </div>
    </div>
  );
}
