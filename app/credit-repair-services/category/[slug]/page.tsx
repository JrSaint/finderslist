...
import type { CreditRepairCategory } from "@/lib/credit-repair-services";
...
export async function generateStaticParams() {
  return getAllCreditRepairCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = CREDIT_REPAIR_CATEGORIES[slug as CreditRepairCategory];
  if (!category) return {};
  const tools = getCreditRepairToolsByCategory(slug as CreditRepairCategory);
  return {
...
export default async function CreditRepairCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!CREDIT_REPAIR_CATEGORIES[slug as CreditRepairCategory]) notFound();

  const category = CREDIT_REPAIR_CATEGORIES[slug as CreditRepairCategory];
  const tools = getCreditRepairToolsByCategory(slug as CreditRepairCategory);
  const allCategories = getAllCreditRepairCategories();
  const totalTools = getAllCreditRepairTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/credit-repair-services/category/${cat}`}
