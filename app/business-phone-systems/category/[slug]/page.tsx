...
import type { BusinessPhoneCategory } from "@/lib/business-phone-systems";
...
export async function generateStaticParams() {
  return getAllBusinessPhoneCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = BUSINESS_PHONE_CATEGORIES[slug as BusinessPhoneCategory];
  if (!category) return {};
  const tools = getBusinessPhoneToolsByCategory(slug as BusinessPhoneCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "business phone systems", category.label],
    alternates: { canonical: `https://www.finderslist.com/business-phone-systems/category/${slug}` },
  };
}

export default async function BusinessPhoneCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!BUSINESS_PHONE_CATEGORIES[slug as BusinessPhoneCategory]) notFound();

  const category = BUSINESS_PHONE_CATEGORIES[slug as BusinessPhoneCategory];
  const tools = getBusinessPhoneToolsByCategory(slug as BusinessPhoneCategory);
  const allCategories = getAllBusinessPhoneCategories();
  const totalTools = getAllBusinessPhoneTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/business-phone-systems/category/${cat}`}
