...
import {
  getFeaturedLifeInsuranceTools,
  getAllLifeInsuranceTools,
  getAllLifeInsuranceCategories,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/life-insurance" } };
  }
  return {
    title: "Best Life Insurance Companies (2026) — 15+ Providers Reviewed",
    description: "Compare the best life insurance companies for term, whole, and universal life policies. Reviews of Haven Life, Bestow, Northwestern Mutual, and more.",
    keywords: ["life insurance","best life insurance 2026","term life insurance","whole life insurance","life insurance quotes"],
    alternates: { canonical: "https://www.finderslist.com/life-insurance" },
  };
}

export default async function LifeInsurancePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedLifeInsuranceTools();
  const allTools = getAllLifeInsuranceTools();
  const categories = getAllLifeInsuranceCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterLifeInsuranceTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Life Insurance Companies", item: "https://www.finderslist.com/life-insurance" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/life-insurance/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (LIFE_INSURANCE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated life insurance
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of life insurance. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/life-insurance" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/life-insurance" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/life-insurance" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/life-insurance" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = LIFE_INSURANCE_CATEGORIES[cat];
                  const count = getLifeInsuranceCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/life-insurance/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/life-insurance" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="life-insurance" />
      </div>
    </div>
  );
}
