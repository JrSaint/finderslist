...
import type { LifeInsuranceCategory } from "@/lib/life-insurance";
...
export async function generateStaticParams() {
  return getAllLifeInsuranceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = LIFE_INSURANCE_CATEGORIES[slug as LifeInsuranceCategory];
  if (!category) return {};
  const tools = getLifeInsuranceToolsByCategory(slug as LifeInsuranceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "life insurance", category.label],
    alternates: { canonical: `https://www.finderslist.com/life-insurance/category/${slug}` },
  };
}

export default async function LifeInsuranceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!LIFE_INSURANCE_CATEGORIES[slug as LifeInsuranceCategory]) notFound();

  const category = LIFE_INSURANCE_CATEGORIES[slug as LifeInsuranceCategory];
  const tools = getLifeInsuranceToolsByCategory(slug as LifeInsuranceCategory);
  const allCategories = getAllLifeInsuranceCategories();
  const totalTools = getAllLifeInsuranceTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/life-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/life-insurance" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
