...
import {
  getFeaturedStudentLoanTools,
  getAllStudentLoanTools,
  getAllStudentLoanCategories,
  STUDENT_LOAN_CATEGORIES,
  getStudentLoanCategoryCount,
  filterStudentLoanTools,
} from "@/lib/student-loans";
import { STUDENT_LOAN_EDITORIAL } from "@/data/student-loans";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/student-loans" } };
  }
  return {
    title: "Best Student Loans & Refinancing (2026) — 15+ Lenders Compared",
    description: "Compare the best student loan lenders and refinancing options. Reviews of SoFi, Earnest, College Ave, Sallie Mae, and more.",
    keywords: ["student loans","student loan refinancing 2026","private student loans","best student loan rates","college loans"],
    alternates: { canonical: "https://www.finderslist.com/student-loans" },
  };
}

export default async function StudentLoanPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedStudentLoanTools();
  const allTools = getAllStudentLoanTools();
  const categories = getAllStudentLoanCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterStudentLoanTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Student Loans & Refinancing", item: "https://www.finderslist.com/student-loans" },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Student Loans & Refinancing",
    description: "Compare the best student loan lenders and refinancing options. Reviews of SoFi, Earnest, College Ave, Sallie Mae, and more.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/student-loans/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (STUDENT_LOAN_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated student loans
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of student loans. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/student-loans" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={STUDENT_LOAN_EDITORIAL.title}
            intro={STUDENT_LOAN_EDITORIAL.intro}
            buyerGuide={STUDENT_LOAN_EDITORIAL.buyerGuide}
            faq={STUDENT_LOAN_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/student-loans" />
          <ComparisonTable
            tools={featured as never[]}
            categories={STUDENT_LOAN_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/student-loans"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/student-loans" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/student-loans" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = STUDENT_LOAN_CATEGORIES[cat];
                  const count = getStudentLoanCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/student-loans/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/student-loans" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="student-loans" />
      </div>
    </div>
  );
}
