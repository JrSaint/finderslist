...
import type { StudentLoanCategory } from "@/lib/student-loans";
...
export async function generateStaticParams() {
  return getAllStudentLoanCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = STUDENT_LOAN_CATEGORIES[slug as StudentLoanCategory];
  if (!category) return {};
  const tools = getStudentLoanToolsByCategory(slug as StudentLoanCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "student loans", category.label],
    alternates: { canonical: `https://www.finderslist.com/student-loans/category/${slug}` },
  };
}

export default async function StudentLoanCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!STUDENT_LOAN_CATEGORIES[slug as StudentLoanCategory]) notFound();

  const category = STUDENT_LOAN_CATEGORIES[slug as StudentLoanCategory];
  const tools = getStudentLoanToolsByCategory(slug as StudentLoanCategory);
  const allCategories = getAllStudentLoanCategories();
  const totalTools = getAllStudentLoanTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/student-loans/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/student-loans" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
