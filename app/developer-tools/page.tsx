...
import {
  getFeaturedDeveloperTools,
  getAllDeveloperTools,
  getAllDeveloperCategories,
  DEVELOPER_CATEGORIES,
  getDeveloperCategoryCount,
  filterDeveloperTools,
  DEVELOPER_ROLES,
} from "@/lib/developer-tools";
import { DEVELOPER_EDITORIAL } from "@/data/developer-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/developer-tools" } };
  }
  return {
    title: "Best Developer Tools & DevOps Directory (2026) — 30+ Tools Reviewed",
    ...
    alternates: { canonical: "https://www.finderslist.com/developer-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(DEVELOPER_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function DeveloperToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedDeveloperTools();
  const allTools = getAllDeveloperTools();
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (DEVELOPER_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-zinc-500/30 bg-zinc-500/10 px-4 py-1.5 text-sm text-zinc-300 backdrop-blur-sm">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/developer-tools" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              ...
              accentClass="bg-zinc-600 border-zinc-500 shadow-zinc-500/20"
            />
          </Suspense>
        </section>
        ...
        {!isFiltering && (
          <EditorialSection
            title={DEVELOPER_EDITORIAL.title}
            intro={DEVELOPER_EDITORIAL.intro}
            buyerGuide={DEVELOPER_EDITORIAL.buyerGuide}
            faq={DEVELOPER_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/developer-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DEVELOPER_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/developer-tools"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/developer-tools" className="text-zinc-400 hover:text-zinc-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/developer-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = DEVELOPER_CATEGORIES[cat];
                  const count = getDeveloperCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/developer-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-zinc-500/30 transition-all group">
                      <span className="text-2xl">{info.emoji}</span>
                      <span className="text-sm font-medium text-white group-hover:text-zinc-300 transition-colors leading-tight">{info.label}</span>
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {featured.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/developer-tools" />))}
              </div>
            </section>
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/developer-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="developer-tools" />
      </div>
    </div>
  );
}
