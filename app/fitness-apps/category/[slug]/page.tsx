...
import type { FitnessAppCategory } from "@/lib/fitness-apps";
...
export async function generateStaticParams() {
  return getAllFitnessAppCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = FITNESS_APP_CATEGORIES[slug as FitnessAppCategory];
  if (!category) return {};
  const tools = getFitnessAppToolsByCategory(slug as FitnessAppCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "fitness apps", category.label],
    alternates: { canonical: `https://www.finderslist.com/fitness-apps/category/${slug}` },
  };
}

export default async function FitnessAppCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!FITNESS_APP_CATEGORIES[slug as FitnessAppCategory]) notFound();

  const category = FITNESS_APP_CATEGORIES[slug as FitnessAppCategory];
  const tools = getFitnessAppToolsByCategory(slug as FitnessAppCategory);
  const allCategories = getAllFitnessAppCategories();
  const totalTools = getAllFitnessAppTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/fitness-apps/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/fitness-apps/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/fitness-apps" className="block w-full rounded-lg bg-red-600 hover:bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
