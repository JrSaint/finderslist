...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/fitness-apps" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/fitness-apps" },
  };
}
...
export default async function FitnessAppPage({ searchParams }: Props) {
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (FITNESS_APP_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/fitness-apps" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/fitness-apps" />
          <ComparisonTable
        ...
        <RelatedDirectories currentSlug="fitness-apps" />
      </div>
    </div>
  );
}
