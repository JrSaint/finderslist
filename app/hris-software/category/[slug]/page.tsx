...
import type { HRISCategory } from "@/lib/hris-software";
...
export async function generateStaticParams() {
  return getAllHRISCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = HRIS_CATEGORIES[slug as HRISCategory];
  if (!category) return {};
  const tools = getHRISToolsByCategory(slug as HRISCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "hris software", category.label],
    alternates: { canonical: `https://www.finderslist.com/hris-software/category/${slug}` },
  };
}

export default async function HRISCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!HRIS_CATEGORIES[slug as HRISCategory]) notFound();

  const category = HRIS_CATEGORIES[slug as HRISCategory];
  const tools = getHRISToolsByCategory(slug as HRISCategory);
  const allCategories = getAllHRISCategories();
  const totalTools = getAllHRISTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/hris-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/hris-software" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
