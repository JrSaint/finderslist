...
import {
  getFeaturedHRISTools,
  getAllHRISTools,
  getAllHRISCategories,
  HRIS_CATEGORIES,
  getHRISCategoryCount,
  filterHRISTools,
} from "@/lib/hris-software";
import { HRIS_EDITORIAL } from "@/data/hris-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/hris-software" } };
  }
  return {
    title: "Best HRIS Software (2026) — 20+ Platforms Reviewed",
    description: "Compare the best HRIS software for managing employee data, onboarding, compliance, and people analytics. Reviews of BambooHR, Workday, Rippling, and more.",
    keywords: ["hris software","best hris 2026","human resource information system","hr management software","people management platform"],
    alternates: { canonical: "https://www.finderslist.com/hris-software" },
  };
}

export default async function HRISPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedHRISTools();
  const allTools = getAllHRISTools();
  const categories = getAllHRISCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterHRISTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/hris-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (HRIS_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-sm text-violet-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated hris software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of hris software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/hris-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={HRIS_EDITORIAL.title}
            intro={HRIS_EDITORIAL.intro}
            buyerGuide={HRIS_EDITORIAL.buyerGuide}
            faq={HRIS_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/hris-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={HRIS_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/hris-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/hris-software" className="text-violet-400 hover:text-violet-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/hris-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = HRIS_CATEGORIES[cat];
                  const count = getHRISCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/hris-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/hris-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="hris-software" />
      </div>
    </div>
  );
}
