...
import {
  getFeaturedBusinessLoanTools,
  getAllBusinessLoanTools,
  getAllBusinessLoanCategories,
  BUSINESS_LOAN_CATEGORIES,
  getBusinessLoanCategoryCount,
  filterBusinessLoanTools,
} from "@/lib/business-loans";
import { BUSINESS_LOAN_EDITORIAL } from "@/data/business-loans";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/business-loans" } };
  }
  return {
    title: "Best Business Loans (2026) — 15+ Lenders Compared",
    description: "Compare the best business loan providers for term loans, SBA loans, lines of credit, and equipment financing.",
    keywords: ["business loans", "small business loans", "business financing", "working capital", "SBA loans", "business line of credit"],
    alternates: { canonical: "https://www.finderslist.com/business-loans" },
  };
}

export default async function BusinessLoansPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedBusinessLoanTools();
  const allTools = getAllBusinessLoanTools();
  const categories = getAllBusinessLoanCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterBusinessLoanTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/business-loans/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (BUSINESS_LOAN_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,rgba(22,163,74,0.2),transparent)] pointer-events-none" />
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 text-sm text-green-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated business loan lenders
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of business loan lenders. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/business-loans" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={BUSINESS_LOAN_EDITORIAL.title}
            intro={BUSINESS_LOAN_EDITORIAL.intro}
            buyerGuide={BUSINESS_LOAN_EDITORIAL.buyerGuide}
            faq={BUSINESS_LOAN_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/business-loans" />
          <ComparisonTable
            tools={featured as never[]}
            categories={BUSINESS_LOAN_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/business-loans"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/business-loans" className="text-green-400 hover:text-green-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/business-loans" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = BUSINESS_LOAN_CATEGORIES[cat];
                  const count = getBusinessLoanCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/business-loans/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/business-loans" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="business-loans" />
      </div>
    </div>
  );
}
