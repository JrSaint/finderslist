...
import {
  getCriminalDefenseLawyerToolBySlug,
  getAllCriminalDefenseLawyerTools,
  getCriminalDefenseLawyerToolsByCategory,
  CRIMINAL_DEFENSE_LAWYER_CATEGORIES,
  CRIMINAL_DEFENSE_LAWYER_PRICING_LABELS,
  CRIMINAL_DEFENSE_LAWYER_PRICING_COLORS,
...
export async function generateStaticParams() {
  return getAllCriminalDefenseLawyerTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getCriminalDefenseLawyerToolBySlug(slug);
  if (!tool) return {};
  const category = CRIMINAL_DEFENSE_LAWYER_CATEGORIES[tool.category];
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/criminal-defense-lawyers/tools/${tool.slug}` },
  };
}

export default async function CriminalDefenseLawyerToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getCriminalDefenseLawyerToolBySlug(slug);
  if (!tool) notFound();

  const category = CRIMINAL_DEFENSE_LAWYER_CATEGORIES[tool.category];
  const relatedTools = getCriminalDefenseLawyerToolsByCategory(tool.category).filter((t) => t.slug !== tool.slug).slice(0, 4);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Criminal Defense Lawyers", item: "https://www.finderslist.com/criminal-defense-lawyers" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/criminal-defense-lawyers/category/${tool.category}` },
      { "@type": "ListItem", position: 4, name: tool.name },
    ],
  };

  const pricingDetail = {
    free: "Free consultation — most criminal defense firms offer free initial case evaluations.",
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href={`/criminal-defense-lawyers/category/${tool.category}`} className="hover:text-slate-300 transition-colors">{category.label}</Link>
        <span>/</span>
          ...
          <div className={`rounded-2xl overflow-hidden mb-8 bg-gradient-to-br ${category.gradient} border border-white/10`}>
            <div className="p-8">
              ...
              <div className="flex flex-wrap gap-2">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${CRIMINAL_DEFENSE_LAWYER_PRICING_COLORS[tool.pricing]}`}>
                  {CRIMINAL_DEFENSE_LAWYER_PRICING_LABELS[tool.pricing]}
                </span>
          ...
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-slate-300">{CRIMINAL_DEFENSE_LAWYER_PRICING_LABELS[tool.pricing]}</span>
              </div>
              {tool.domain && (
                <div className="flex justify-between">
          ...
          {relatedTools.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {relatedTools.map((t) => (
                  <ToolCard key={t.slug} tool={t as never} variant="compact" basePath="/criminal-defense-lawyers" />
                ))}
              </div>
            </div>
          )}
