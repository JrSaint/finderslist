...
export default async function CriminalDefenseLawyersPage({ searchParams }: Props) {
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/criminal-defense-lawyers/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/criminal-defense-lawyers" />
          <ComparisonTable
        ...
        <RelatedDirectories currentSlug="criminal-defense-lawyers" />
      </div>
    </div>
  );
}
