...
import {
  getFeaturedRealEstateTools,
  getAllRealEstateTools,
  getAllRealEstateCategories,
  REAL_ESTATE_CATEGORIES,
  getRealEstateCategoryCount,
  filterRealEstateTools,
} from "@/lib/real-estate-software";
import { REAL_ESTATE_EDITORIAL } from "@/data/real-estate-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/real-estate-software" } };
  }
  return {
    title: "Best Real Estate Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best real estate software for agents, brokerages, and investors. Reviews of Follow Up Boss, kvCORE, Dotloop, and more.",
    keywords: ["real estate software","real estate crm 2026","real estate technology","transaction management","real estate lead generation"],
    alternates: { canonical: "https://www.finderslist.com/real-estate-software" },
  };
}

export default async function RealEstatePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedRealEstateTools();
  const allTools = getAllRealEstateTools();
  const categories = getAllRealEstateCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterRealEstateTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/real-estate-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (REAL_ESTATE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated real estate software
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={REAL_ESTATE_EDITORIAL.title}
            intro={REAL_ESTATE_EDITORIAL.intro}
            buyerGuide={REAL_ESTATE_EDITORIAL.buyerGuide}
            faq={REAL_ESTATE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/real-estate-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={REAL_ESTATE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/real-estate-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/real-estate-software" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/real-estate-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = REAL_ESTATE_CATEGORIES[cat];
                  const count = getRealEstateCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/real-estate-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/real-estate-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="real-estate-software" />
      </div>
    </div>
  );
}
