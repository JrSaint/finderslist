...
import type { RealEstateCategory } from "@/lib/real-estate-software";
...
export async function generateStaticParams() {
  return getAllRealEstateCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = REAL_ESTATE_CATEGORIES[slug as RealEstateCategory];
  if (!category) return {};
  const tools = getRealEstateToolsByCategory(slug as RealEstateCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "real estate software", category.label],
    alternates: { canonical: `https://www.finderslist.com/real-estate-software/category/${slug}` },
  };
}
...
export default async function RealEstateCategoryPage({ params }: Props) {
  ...
  if (!REAL_ESTATE_CATEGORIES[slug as RealEstateCategory]) notFound();

  const category = REAL_ESTATE_CATEGORIES[slug as RealEstateCategory];
  const tools = getRealEstateToolsByCategory(slug as RealEstateCategory);
  const allCategories = getAllRealEstateCategories();
  const totalTools = getAllRealEstateTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/real-estate-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/real-estate-software" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
