...
import {
  getExpenseManagementToolBySlug,
  getAllExpenseManagementTools,
  getExpenseManagementToolsByCategory,
  EXPENSE_MANAGEMENT_CATEGORIES,
  EXPENSE_MANAGEMENT_PRICING_LABELS,
  EXPENSE_MANAGEMENT_PRICING_COLORS,
} from "@/lib/expense-management-software";
...
export async function generateStaticParams() {
  return getAllExpenseManagementTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getExpenseManagementToolBySlug(slug);
  if (!tool) return {};
  const category = EXPENSE_MANAGEMENT_CATEGORIES[tool.category];
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/expense-management-software/tools/${tool.slug}` },
  };
}

export default async function ExpenseManagementToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getExpenseManagementToolBySlug(slug);
  if (!tool) notFound();

  const category = EXPENSE_MANAGEMENT_CATEGORIES[tool.category];
  const relatedTools = getExpenseManagementToolsByCategory(tool.category).filter((t) => t.slug !== tool.slug).slice(0, 4);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Expense Management Software", item: "https://www.finderslist.com/expense-management-software" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/expense-management-software/category/${tool.category}` },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href="/expense-management-software" className="hover:text-slate-300 transition-colors">Expense Management Software</Link>
        <span>/</span>
        <Link href={`/expense-management-software/category/${tool.category}`} className="hover:text-slate-300 transition-colors">{category.label}</Link>
        <span>/</span>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 min-w-0">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    {tool.featured && <span className="text-xs bg-rose-500/30 text-rose-200 border border-rose-400/30 rounded-full px-2.5 py-0.5">Featured</span>}
                    <span className={`text-xs border rounded-full px-2.5 py-0.5 ${EXPENSE_MANAGEMENT_PRICING_COLORS[tool.pricing]}`}>{EXPENSE_MANAGEMENT_PRICING_LABELS[tool.pricing]}</span>
                  </div>
                  <p className="text-lg text-white/80 mb-3 leading-snug">{tool.tagline}</p>
                  <Link href={`/expense-management-software/category/${tool.category}`} className="inline-flex items-center gap-1.5 text-sm text-white/60 hover:text-white/90 transition-colors">
          ...
          <div className="sm:hidden mb-6">
            <a href={isDead ? tool.url : visitUrl} target="_blank" rel={tool.affiliateUrl ? "sponsored nofollow noopener noreferrer" : "noopener noreferrer"} aria-disabled={isDead || undefined} className={`flex items-center justify-center gap-2 rounded-xl bg-rose-600 hover:bg-rose-500 px-6 py-3 font-semibold text-white transition-colors${isDead ? " opacity-50 grayscale pointer-events-none" : ""}`}>
              {isDead ? "No longer available" : <>Visit {tool.name}</>}
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
            </a>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              {tool.cons && tool.cons.length > 0 && (
                <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-5">
                  ...
                  <ul className="space-y-2">{tool.cons.map((con, i) => (<li key={i} className="flex items-start gap-2 text-sm text-slate-300"><span className="text-rose-500 mt-0.5 flex-shrink-0">!</span>{con}</li>))}</ul>
                </div>
              )}
            </div>
          )}
              ...
              <div className="space-y-3">
                {tool.useCases.map((useCase, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/30 flex items-center justify-center flex-shrink-0 text-xs font-bold text-rose-400 mt-0.5">{i + 1}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <p className="text-sm font-semibold text-white mb-1">{EXPENSE_MANAGEMENT_PRICING_LABELS[tool.pricing]}</p>
          ...
          {relatedTools.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href={`/expense-management-software/category/${tool.category}`} className="text-xs text-rose-400 hover:text-rose-300 transition-colors">See all →</Link>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTools.map((t) => (<ToolCard key={t.slug} tool={t as never} basePath="/expense-management-software" />))}
              </div>
            </section>
          )}
        </div>
        ...
        <aside className="lg:w-64 flex-shrink-0">
          <div className="sticky top-24 space-y-4">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <a href={isDead ? tool.url : visitUrl} target="_blank" rel={tool.affiliateUrl ? "sponsored nofollow noopener noreferrer" : "noopener noreferrer"} aria-disabled={isDead || undefined} className={`block w-full rounded-lg bg-rose-600 hover:bg-rose-500 px-4 py-2.5 text-sm font-medium text-white transition-colors text-center${isDead ? " opacity-50 grayscale pointer-events-none" : ""}`}>{isDead ? "No longer available" : <>Open {tool.name} →</>}</a>
              <p className="text-xs text-slate-600 mt-2 text-center">{EXPENSE_MANAGEMENT_PRICING_LABELS[tool.pricing]} · {category.label}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              <p className="text-sm font-medium text-white mb-2">💰 Pricing</p>
              <span className={`inline-flex text-xs border rounded-full px-2.5 py-1 mb-2 ${EXPENSE_MANAGEMENT_PRICING_COLORS[tool.pricing]}`}>{EXPENSE_MANAGEMENT_PRICING_LABELS[tool.pricing]}</span>
              <p className="text-xs text-slate-500 leading-relaxed">{isDead ? "" : tool.startingPrice ? `From ${tool.startingPrice}. ` : ""}{pricingText}</p>
            </div>
            {tool.pros && tool.pros.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <Link href={`/expense-management-software/category/${tool.category}`} className="block w-full rounded-lg border border-white/10 hover:border-rose-500/30 hover:bg-slate-800 px-4 py-2.5 text-sm text-slate-400 hover:text-white transition-all text-center">See all tools →</Link>
            </div>
          </div>
        </aside>
      </div>
