...
import {
  getFeaturedAutoInsuranceTools,
  getAllAutoInsuranceTools,
  getAllAutoInsuranceCategories,
  AUTO_INSURANCE_CATEGORIES,
  getAutoInsuranceCategoryCount,
  filterAutoInsuranceTools,
} from "@/lib/auto-insurance";
import { AUTO_INSURANCE_EDITORIAL } from "@/data/auto-insurance";
...
export default async function AutoInsurancePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedAutoInsuranceTools();
  const allTools = getAllAutoInsuranceTools();
  const categories = getAllAutoInsuranceCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterAutoInsuranceTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Auto Insurance Companies", item: "https://www.finderslist.com/auto-insurance" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/auto-insurance/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-4 py-1.5 text-sm text-blue-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated auto insurance
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of auto insurance. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/auto-insurance" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={AUTO_INSURANCE_EDITORIAL.title}
            intro={AUTO_INSURANCE_EDITORIAL.intro}
            buyerGuide={AUTO_INSURANCE_EDITORIAL.buyerGuide}
            faq={AUTO_INSURANCE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/auto-insurance" />
          <ComparisonTable
            tools={featured as never[]}
            categories={AUTO_INSURANCE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/auto-insurance"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/auto-insurance" className="text-blue-400 hover:text-blue-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/auto-insurance" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = AUTO_INSURANCE_CATEGORIES[cat];
                  const count = getAutoInsuranceCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/auto-insurance/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/auto-insurance" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="auto-insurance" />
      </div>
    </div>
  );
}
