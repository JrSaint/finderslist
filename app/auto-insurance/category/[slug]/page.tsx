...
import type { AutoInsuranceCategory } from "@/lib/auto-insurance";
...
export async function generateStaticParams() {
  return getAllAutoInsuranceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = AUTO_INSURANCE_CATEGORIES[slug as AutoInsuranceCategory];
  if (!category) return {};
  const tools = getAutoInsuranceToolsByCategory(slug as AutoInsuranceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "auto insurance", category.label],
    alternates: { canonical: `https://www.finderslist.com/auto-insurance/category/${slug}` },
  };
}

export default async function AutoInsuranceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!AUTO_INSURANCE_CATEGORIES[slug as AutoInsuranceCategory]) notFound();

  const category = AUTO_INSURANCE_CATEGORIES[slug as AutoInsuranceCategory];
  const tools = getAutoInsuranceToolsByCategory(slug as AutoInsuranceCategory);
  const allCategories = getAllAutoInsuranceCategories();
  const totalTools = getAllAutoInsuranceTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/auto-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/auto-insurance" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
