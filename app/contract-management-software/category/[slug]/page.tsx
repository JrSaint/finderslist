...
import type { ContractManagementCategory } from "@/lib/contract-management-software";
...
export async function generateStaticParams() {
  return getAllContractManagementCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = CONTRACT_MANAGEMENT_CATEGORIES[slug as ContractManagementCategory];
  if (!category) return {};
  const tools = getContractManagementToolsByCategory(slug as ContractManagementCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "contract management software", category.label],
    alternates: { canonical: `https://www.finderslist.com/contract-management-software/category/${slug}` },
  };
}

export default async function ContractManagementCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!CONTRACT_MANAGEMENT_CATEGORIES[slug as ContractManagementCategory]) notFound();

  const category = CONTRACT_MANAGEMENT_CATEGORIES[slug as ContractManagementCategory];
  const tools = getContractManagementToolsByCategory(slug as ContractManagementCategory);
  const allCategories = getAllContractManagementCategories();
  const totalTools = getAllContractManagementTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/contract-management-software/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/contract-management-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/contract-management-software" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
