...
import {
  getFeaturedIdentityTheftProtectionTools,
  getAllIdentityTheftProtectionTools,
  getAllIdentityTheftProtectionCategories,
  IDENTITY_THEFT_PROTECTION_CATEGORIES,
  getIdentityTheftProtectionCategoryCount,
  filterIdentityTheftProtectionTools,
} from "@/lib/identity-theft-protection";
import { IDENTITY_THEFT_PROTECTION_EDITORIAL } from "@/data/identity-theft-protection";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/identity-theft-protection" } };
  }
  return {
    title: "Best Identity Theft Protection Services (2026) — 15+ Services Compared",
    description: "Compare the best identity theft protection services for credit monitoring, dark web surveillance, family plans, and identity restoration with honest reviews.",
    keywords: ["identity theft protection", "best identity theft protection services", "credit monitoring", "dark web monitoring", "identity theft insurance", "identity restoration"],
    alternates: { canonical: "https://www.finderslist.com/identity-theft-protection" },
  };
}

export default async function IdentityTheftProtectionPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedIdentityTheftProtectionTools();
  const allTools = getAllIdentityTheftProtectionTools();
  const categories = getAllIdentityTheftProtectionCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterIdentityTheftProtectionTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Identity Theft Protection Services", item: "https://www.finderslist.com/identity-theft-protection" },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Identity Theft Protection Services",
    description: "Compare the best identity theft protection services for credit monitoring, dark web surveillance, family plans, and identity restoration with honest reviews.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/identity-theft-protection/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (IDENTITY_THEFT_PROTECTION_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated identity theft protection services
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-5">
            <span className="text-white">Find the Right </span>
            <span className="bg-gradient-to-r from-indigo-400 via-indigo-400 to-blue-300 bg-clip-text text-transparent">
              Identity Theft Protection
            </span>
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of identity theft protection services. Compare top providers with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/identity-theft-protection" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/identity-theft-protection"
        ...
        {!isFiltering && (
          <EditorialSection
            title={IDENTITY_THEFT_PROTECTION_EDITORIAL.title}
            intro={IDENTITY_THEFT_PROTECTION_EDITORIAL.intro}
            buyerGuide={IDENTITY_THEFT_PROTECTION_EDITORIAL.buyerGuide}
            faq={IDENTITY_THEFT_PROTECTION_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/identity-theft-protection" />
          <ComparisonTable
            tools={featured as never[]}
            categories={IDENTITY_THEFT_PROTECTION_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/identity-theft-protection"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/identity-theft-protection" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getIdentityTheftProtectionCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/identity-theft-protection/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/identity-theft-protection" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="identity-theft-protection" />
      </div>
    </div>
  );
}
