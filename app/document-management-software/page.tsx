...
import {
  getFeaturedDocumentManagementTools,
  getAllDocumentManagementTools,
  getAllDocumentManagementCategories,
  DOCUMENT_MANAGEMENT_CATEGORIES,
  getDocumentManagementCategoryCount,
  filterDocumentManagementTools,
} from "@/lib/document-management-software";
import { DOCUMENT_MANAGEMENT_EDITORIAL } from "@/data/document-management-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/document-management-software" } };
  }
  return {
    title: "Best Document Management Software (2026) — 15+ Solutions Compared",
    description: "Compare the best document management systems for businesses. Reviews of SharePoint, DocuSign, Google Workspace, Box, and more.",
    keywords: ["document management software","dms systems 2026","document storage","file management","enterprise content management"],
    alternates: { canonical: "https://www.finderslist.com/document-management-software" },
  };
}

export default async function DocumentManagementPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedDocumentManagementTools();
  const allTools = getAllDocumentManagementTools();
  const categories = getAllDocumentManagementCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterDocumentManagementTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/document-management-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (DOCUMENT_MANAGEMENT_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-4 py-1.5 text-sm text-sky-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated document management software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of document management software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/document-management-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/document-management-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={DOCUMENT_MANAGEMENT_EDITORIAL.title}
            intro={DOCUMENT_MANAGEMENT_EDITORIAL.intro}
            buyerGuide={DOCUMENT_MANAGEMENT_EDITORIAL.buyerGuide}
            faq={DOCUMENT_MANAGEMENT_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/document-management-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DOCUMENT_MANAGEMENT_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/document-management-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/document-management-software" className="text-sky-400 hover:text-sky-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/document-management-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = DOCUMENT_MANAGEMENT_CATEGORIES[cat];
                  const count = getDocumentManagementCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/document-management-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/document-management-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="document-management-software" />
      </div>
    </div>
  );
}
