...
import {
  getFeaturedPOSSystemTools,
  getAllPOSSystemTools,
  getAllPOSSystemCategories,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/pos-systems" } };
  }
  return {
    title: "Best POS Systems (2026) — 15+ Point-of-Sale Solutions Compared",
    description: "Compare the best POS systems for retail, restaurants, and service businesses. Reviews of Square, Toast, Clover, Shopify POS, and more.",
    keywords: ["pos systems","best point of sale 2026","restaurant pos","retail pos","payment terminals"],
    alternates: { canonical: "https://www.finderslist.com/pos-systems" },
  };
}

export default async function POSSystemPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedPOSSystemTools();
  const allTools = getAllPOSSystemTools();
  const categories = getAllPOSSystemCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterPOSSystemTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/pos-systems/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (POS_SYSTEM_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-sm text-violet-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated pos systems
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of pos systems. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/pos-systems" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/pos-systems" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/pos-systems" className="text-violet-400 hover:text-violet-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/pos-systems" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = POS_SYSTEM_CATEGORIES[cat];
                  const count = getPOSSystemCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/pos-systems/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/pos-systems" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="pos-systems" />
      </div>
    </div>
  );
}
