...
import type { POSSystemCategory } from "@/lib/pos-systems";
...
export async function generateStaticParams() {
  return getAllPOSSystemCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = POS_SYSTEM_CATEGORIES[slug as POSSystemCategory];
  if (!category) return {};
  const tools = getPOSSystemToolsByCategory(slug as POSSystemCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "pos systems", category.label],
    alternates: { canonical: `https://www.finderslist.com/pos-systems/category/${slug}` },
  };
}

export default async function POSSystemCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!POS_SYSTEM_CATEGORIES[slug as POSSystemCategory]) notFound();

  const category = POS_SYSTEM_CATEGORIES[slug as POSSystemCategory];
  const tools = getPOSSystemToolsByCategory(slug as POSSystemCategory);
  const allCategories = getAllPOSSystemCategories();
  const totalTools = getAllPOSSystemTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/pos-systems/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/pos-systems" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
