...
import { BUSINESS_INSURANCE_EDITORIAL } from "@/data/business-insurance";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/business-insurance" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/business-insurance" },
  };
}
...
export default async function BusinessInsurancePage({ searchParams }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Business Insurance Companies", item: "https://www.finderslist.com/business-insurance" },
    ],
  };
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (BUSINESS_INSURANCE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/business-insurance" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={BUSINESS_INSURANCE_EDITORIAL.title}
            intro={BUSINESS_INSURANCE_EDITORIAL.intro}
            buyerGuide={BUSINESS_INSURANCE_EDITORIAL.buyerGuide}
            faq={BUSINESS_INSURANCE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/business-insurance" />
          <ComparisonTable
            tools={featured as never[]}
            categories={BUSINESS_INSURANCE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/business-insurance"
          />
          </>
        )}
        ...
        <RelatedDirectories currentSlug="business-insurance" />
      </div>
    </div>
  );
}
