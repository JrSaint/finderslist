...
import type { BusinessInsuranceCategory } from "@/lib/business-insurance";
...
export async function generateStaticParams() {
  return getAllBusinessInsuranceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = BUSINESS_INSURANCE_CATEGORIES[slug as BusinessInsuranceCategory];
  if (!category) return {};
  const tools = getBusinessInsuranceToolsByCategory(slug as BusinessInsuranceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "business insurance", category.label],
    alternates: { canonical: `https://www.finderslist.com/business-insurance/category/${slug}` },
  };
}

export default async function BusinessInsuranceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!BUSINESS_INSURANCE_CATEGORIES[slug as BusinessInsuranceCategory]) notFound();

  const category = BUSINESS_INSURANCE_CATEGORIES[slug as BusinessInsuranceCategory];
  const tools = getBusinessInsuranceToolsByCategory(slug as BusinessInsuranceCategory);
  const allCategories = getAllBusinessInsuranceCategories();
  const totalTools = getAllBusinessInsuranceTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/business-insurance/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/business-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/business-insurance" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
