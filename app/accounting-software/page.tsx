...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/accounting-software" } };
  }
  return {
    title: "Best Accounting Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best accounting software for small businesses, freelancers, and enterprises. Reviews of QuickBooks, Xero, FreshBooks, and more.",
    keywords: ["accounting software","best accounting software 2026","small business accounting","bookkeeping software","cloud accounting"],
    alternates: { canonical: "https://www.finderslist.com/accounting-software" },
  };
}

export default async function AccountingPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedAccountingTools();
  const allTools = getAllAccountingTools();
  const categories = getAllAccountingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterAccountingTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/accounting-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ACCOUNTING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-4 py-1.5 text-sm text-blue-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated accounting software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of accounting software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/accounting-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/accounting-software" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/accounting-software" className="text-blue-400 hover:text-blue-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/accounting-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ACCOUNTING_CATEGORIES[cat];
                  const count = getAccountingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/accounting-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/accounting-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="accounting-software" />
      </div>
    </div>
  );
}
