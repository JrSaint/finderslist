...
import type { AccountingCategory } from "@/lib/accounting-software";
...
export async function generateStaticParams() {
  return getAllAccountingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ACCOUNTING_CATEGORIES[slug as AccountingCategory];
  if (!category) return {};
  const tools = getAccountingToolsByCategory(slug as AccountingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "accounting software", category.label],
    alternates: { canonical: `https://www.finderslist.com/accounting-software/category/${slug}` },
  };
}

export default async function AccountingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ACCOUNTING_CATEGORIES[slug as AccountingCategory]) notFound();

  const category = ACCOUNTING_CATEGORIES[slug as AccountingCategory];
  const tools = getAccountingToolsByCategory(slug as AccountingCategory);
  const allCategories = getAllAccountingCategories();
  const totalTools = getAllAccountingTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/accounting-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/accounting-software" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
