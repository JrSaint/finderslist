...
import type { RoboAdvisorCategory } from "@/lib/robo-advisors";
...
export async function generateStaticParams() {
  return getAllRoboAdvisorCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ROBO_ADVISOR_CATEGORIES[slug as RoboAdvisorCategory];
  if (!category) return {};
  const tools = getRoboAdvisorToolsByCategory(slug as RoboAdvisorCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "robo-advisors", category.label],
    alternates: { canonical: `https://www.finderslist.com/robo-advisors/category/${slug}` },
  };
}

export default async function RoboAdvisorCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ROBO_ADVISOR_CATEGORIES[slug as RoboAdvisorCategory]) notFound();

  const category = ROBO_ADVISOR_CATEGORIES[slug as RoboAdvisorCategory];
  const tools = getRoboAdvisorToolsByCategory(slug as RoboAdvisorCategory);
  const allCategories = getAllRoboAdvisorCategories();
  const totalTools = getAllRoboAdvisorTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/robo-advisors/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/robo-advisors" className="block w-full rounded-lg bg-cyan-600 hover:bg-cyan-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
