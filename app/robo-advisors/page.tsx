...
import {
  getFeaturedRoboAdvisorTools,
  getAllRoboAdvisorTools,
  getAllRoboAdvisorCategories,
  ROBO_ADVISOR_CATEGORIES,
  getRoboAdvisorCategoryCount,
  filterRoboAdvisorTools,
} from "@/lib/robo-advisors";
import { ROBO_ADVISOR_EDITORIAL } from "@/data/robo-advisors";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/robo-advisors" } };
  }
  return {
    title: "Best Robo-Advisors (2026) — 15+ Automated Investing Platforms Compared",
    description: "Compare the best robo-advisors for automated investing and portfolio management. Reviews of Betterment, Wealthfront, Schwab Intelligent, and more.",
    keywords: ["robo-advisors","best robo advisors 2026","automated investing","portfolio management","passive investing"],
    alternates: { canonical: "https://www.finderslist.com/robo-advisors" },
  };
}

export default async function RoboAdvisorPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedRoboAdvisorTools();
  const allTools = getAllRoboAdvisorTools();
  const categories = getAllRoboAdvisorCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterRoboAdvisorTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/robo-advisors/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ROBO_ADVISOR_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-4 py-1.5 text-sm text-cyan-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated robo-advisors
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of robo-advisors. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/robo-advisors" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={ROBO_ADVISOR_EDITORIAL.title}
            intro={ROBO_ADVISOR_EDITORIAL.intro}
            buyerGuide={ROBO_ADVISOR_EDITORIAL.buyerGuide}
            faq={ROBO_ADVISOR_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/robo-advisors" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ROBO_ADVISOR_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/robo-advisors"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/robo-advisors" className="text-cyan-400 hover:text-cyan-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/robo-advisors" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ROBO_ADVISOR_CATEGORIES[cat];
                  const count = getRoboAdvisorCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/robo-advisors/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/robo-advisors" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="robo-advisors" />
      </div>
    </div>
  );
}
