...
import type { BusinessTaxCategory } from "@/lib/business-tax-software";
...
export async function generateStaticParams() {
  return getAllBusinessTaxCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = BUSINESS_TAX_CATEGORIES[slug as BusinessTaxCategory];
  if (!category) return {};
  const tools = getBusinessTaxToolsByCategory(slug as BusinessTaxCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "business tax software", category.label],
    alternates: { canonical: `https://www.finderslist.com/business-tax-software/category/${slug}` },
  };
}

export default async function BusinessTaxCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!BUSINESS_TAX_CATEGORIES[slug as BusinessTaxCategory]) notFound();

  const category = BUSINESS_TAX_CATEGORIES[slug as BusinessTaxCategory];
  const tools = getBusinessTaxToolsByCategory(slug as BusinessTaxCategory);
  const allCategories = getAllBusinessTaxCategories();
  const totalTools = getAllBusinessTaxTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/business-tax-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/business-tax-software" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
