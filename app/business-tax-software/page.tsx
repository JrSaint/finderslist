...
import {
  getFeaturedBusinessTaxTools,
  getAllBusinessTaxTools,
  getAllBusinessTaxCategories,
  BUSINESS_TAX_CATEGORIES,
  getBusinessTaxCategoryCount,
  filterBusinessTaxTools,
} from "@/lib/business-tax-software";
import { BUSINESS_TAX_EDITORIAL } from "@/data/business-tax-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/business-tax-software" } };
  }
  return {
    title: "Best Business Tax Software (2026) — 18+ Tools Reviewed",
    description: "Compare the best business tax software for filing, sales tax, and tax planning. Reviews of TurboTax Business, Avalara, TaxJar, and more.",
    keywords: ["business tax software","tax software 2026","sales tax automation","tax filing software","business tax preparation"],
    alternates: { canonical: "https://www.finderslist.com/business-tax-software" },
  };
}

export default async function BusinessTaxPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedBusinessTaxTools();
  const allTools = getAllBusinessTaxTools();
  const categories = getAllBusinessTaxCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterBusinessTaxTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/business-tax-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (BUSINESS_TAX_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated business tax software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of business tax software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/business-tax-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/business-tax-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={BUSINESS_TAX_EDITORIAL.title}
            intro={BUSINESS_TAX_EDITORIAL.intro}
            buyerGuide={BUSINESS_TAX_EDITORIAL.buyerGuide}
            faq={BUSINESS_TAX_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/business-tax-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={BUSINESS_TAX_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/business-tax-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/business-tax-software" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/business-tax-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = BUSINESS_TAX_CATEGORIES[cat];
                  const count = getBusinessTaxCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/business-tax-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/business-tax-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="business-tax-software" />
      </div>
    </div>
  );
}
