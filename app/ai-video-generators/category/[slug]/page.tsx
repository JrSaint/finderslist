...
import type { AIVideoCategory } from "@/lib/ai-video-generators";
...
export async function generateStaticParams() {
  return getAllAIVideoCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = AI_VIDEO_CATEGORIES[slug as AIVideoCategory];
  if (!category) return {};
  const tools = getAIVideoToolsByCategory(slug as AIVideoCategory);
  return {
...
export default async function AIVideoCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!AI_VIDEO_CATEGORIES[slug as AIVideoCategory]) notFound();

  const category = AI_VIDEO_CATEGORIES[slug as AIVideoCategory];
  const tools = getAIVideoToolsByCategory(slug as AIVideoCategory);
  const allCategories = getAllAIVideoCategories();
  const totalTools = getAllAIVideoTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/ai-video-generators/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/ai-video-generators" className="block w-full rounded-lg bg-purple-600 hover:bg-purple-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
