...
import {
  getAIVideoToolBySlug,
  getAllAIVideoTools,
  getAIVideoToolsByCategory,
  AI_VIDEO_CATEGORIES,
  AI_VIDEO_PRICING_LABELS,
  AI_VIDEO_PRICING_COLORS,
} from "@/lib/ai-video-generators";
...
export async function generateStaticParams() {
  return getAllAIVideoTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getAIVideoToolBySlug(slug);
  if (!tool) return {};
  const category = AI_VIDEO_CATEGORIES[tool.category];
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/ai-video-generators/tools/${tool.slug}` },
  };
}

export default async function AIVideoToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getAIVideoToolBySlug(slug);
  if (!tool) notFound();

  const category = AI_VIDEO_CATEGORIES[tool.category];
  const relatedTools = getAIVideoToolsByCategory(tool.category).filter((t) => t.slug !== tool.slug).slice(0, 4);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "AI Video Generators", item: "https://www.finderslist.com/ai-video-generators" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/ai-video-generators/category/${tool.category}` },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href="/ai-video-generators" className="hover:text-slate-300 transition-colors">AI Video Generators</Link>
        <span>/</span>
        <Link href={`/ai-video-generators/category/${tool.category}`} className="hover:text-slate-300 transition-colors">{category.label}</Link>
        <span>/</span>
        ...
        <div className="flex-1 min-w-0">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    <span className={`text-xs border rounded-full px-2.5 py-0.5 ${AI_VIDEO_PRICING_COLORS[tool.pricing]}`}>{AI_VIDEO_PRICING_LABELS[tool.pricing]}</span>
                  </div>
                  <p className="text-lg text-white/80 mb-3 leading-snug">{tool.tagline}</p>
                  <Link href={`/ai-video-generators/category/${tool.category}`} className="inline-flex items-center gap-1.5 text-sm text-white/60 hover:text-white/90 transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <p className="text-sm font-semibold text-white mb-1">{AI_VIDEO_PRICING_LABELS[tool.pricing]}</p>
          ...
          {relatedTools.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href={`/ai-video-generators/category/${tool.category}`} className="text-xs text-purple-400 hover:text-purple-300 transition-colors">See all →</Link>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTools.map((t) => (<ToolCard key={t.slug} tool={t as never} basePath="/ai-video-generators" />))}
              </div>
            </section>
          )}
        </div>
        ...
        <aside className="lg:w-64 flex-shrink-0">
          <div className="sticky top-24 space-y-4">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <p className="text-xs text-slate-600 mt-2 text-center">{AI_VIDEO_PRICING_LABELS[tool.pricing]} · {category.label}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              <p className="text-sm font-medium text-white mb-2">💰 Pricing</p>
              <span className={`inline-flex text-xs border rounded-full px-2.5 py-1 mb-2 ${AI_VIDEO_PRICING_COLORS[tool.pricing]}`}>{AI_VIDEO_PRICING_LABELS[tool.pricing]}</span>
              <p className="text-xs text-slate-500 leading-relaxed">{isDead ? "" : tool.startingPrice ? `From ${tool.startingPrice}. ` : ""}{pricingText}</p>
            </div>
            {tool.pros && tool.pros.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <Link href={`/ai-video-generators/category/${tool.category}`} className="block w-full rounded-lg border border-white/10 hover:border-purple-500/30 hover:bg-slate-800 px-4 py-2.5 text-sm text-slate-400 hover:text-white transition-all text-center">See all tools →</Link>
            </div>
          </div>
        </aside>
      </div>
