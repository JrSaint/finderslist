...
import {
  getFeaturedAIVideoTools,
  getAllAIVideoTools,
  getAllAIVideoCategories,
  AI_VIDEO_CATEGORIES,
  getAIVideoCategoryCount,
  filterAIVideoTools,
} from "@/lib/ai-video-generators";
import { AI_VIDEO_EDITORIAL } from "@/data/ai-video-generators";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/ai-video-generators" } };
  }
  return {
    title: "Best AI Video Generators (2026) — 20+ Tools Reviewed",
    description: "Compare the best AI video generators for text-to-video, AI avatars, and AI-powered editing. Reviews of Synthesia, HeyGen, Runway, Pictory, and more.",
    keywords: ["ai video generator","best ai video tools 2026","text to video ai","ai avatar generator","ai video editing"],
    alternates: { canonical: "https://www.finderslist.com/ai-video-generators" },
  };
}

export default async function AIVideoPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedAIVideoTools();
  const allTools = getAllAIVideoTools();
  const categories = getAllAIVideoCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterAIVideoTools({ query: q, pricing, role })
    : allTools;
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (AI_VIDEO_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-purple-500/30 bg-purple-500/10 px-4 py-1.5 text-sm text-purple-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated ai video generators
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of ai video generators. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/ai-video-generators" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={AI_VIDEO_EDITORIAL.title}
            intro={AI_VIDEO_EDITORIAL.intro}
            buyerGuide={AI_VIDEO_EDITORIAL.buyerGuide}
            faq={AI_VIDEO_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ai-video-generators" />
          <ComparisonTable
            tools={featured as never[]}
            categories={AI_VIDEO_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/ai-video-generators"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/ai-video-generators" className="text-purple-400 hover:text-purple-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/ai-video-generators" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = AI_VIDEO_CATEGORIES[cat];
                  const count = getAIVideoCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/ai-video-generators/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/ai-video-generators" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="ai-video-generators" />
      </div>
    </div>
  );
}
