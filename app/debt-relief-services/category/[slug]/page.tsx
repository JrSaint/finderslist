...
import type { DebtReliefCategory } from "@/lib/debt-relief-services";
...
export async function generateStaticParams() {
  return getAllDebtReliefCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = DEBT_RELIEF_CATEGORIES[slug as DebtReliefCategory];
  if (!category) return {};
  const tools = getDebtReliefToolsByCategory(slug as DebtReliefCategory);
  return {
    robots: { index: false, follow: true },
    title: `Best ${category.label} Services (2026) — ${tools.length} Options Compared`,
    description: `Discover the best ${category.label.toLowerCase()} services in 2026. We've curated ${tools.length} top providers with honest reviews, pricing breakdowns, and real-world use cases.`,
    keywords: [`${category.label} services`, `best ${category.label} services 2026`, "debt relief", category.label],
    alternates: { canonical: `https://www.finderslist.com/debt-relief-services/category/${slug}` },
  };
}

export default async function DebtReliefCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!DEBT_RELIEF_CATEGORIES[slug as DebtReliefCategory]) notFound();

  const category = DEBT_RELIEF_CATEGORIES[slug as DebtReliefCategory];
  const tools = getDebtReliefToolsByCategory(slug as DebtReliefCategory);
  const allCategories = getAllDebtReliefCategories();
  const totalTools = getAllDebtReliefTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/debt-relief-services/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/debt-relief-services/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/debt-relief-services" className="block w-full rounded-lg bg-teal-600 hover:bg-teal-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
