...
import { DEBT_RELIEF_EDITORIAL } from "@/data/debt-relief-services";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/debt-relief-services" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/debt-relief-services" },
  };
}
...
export default async function DebtReliefServicesPage({ searchParams }: Props) {
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (DEBT_RELIEF_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/debt-relief-services" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={DEBT_RELIEF_EDITORIAL.title}
            intro={DEBT_RELIEF_EDITORIAL.intro}
            buyerGuide={DEBT_RELIEF_EDITORIAL.buyerGuide}
            faq={DEBT_RELIEF_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/debt-relief-services" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DEBT_RELIEF_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/debt-relief-services"
          />
          </>
        )}
        ...
        <RelatedDirectories currentSlug="debt-relief-services" />
      </div>
    </div>
  );
}
