...
export async function generateStaticParams() {
  return getAllMortgageLenderCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = MORTGAGE_LENDER_CATEGORIES[slug as MortgageLenderCategory];
  if (!category) return {};
...
export default async function MortgageLenderCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!MORTGAGE_LENDER_CATEGORIES[slug as MortgageLenderCategory]) notFound();

  const category = MORTGAGE_LENDER_CATEGORIES[slug as MortgageLenderCategory];
  const tools = getMortgageLenderToolsByCategory(slug as MortgageLenderCategory);
  const allCategories = getAllMortgageLenderCategories();
  const totalTools = getAllMortgageLenderTools().length;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `Best ${category.label} Lenders`,
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/mortgage-lenders/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/mortgage-lenders" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
