...
import {
  getFeaturedEcommercePlatformTools,
  getAllEcommercePlatformTools,
  getAllEcommercePlatformCategories,
  ECOMMERCE_PLATFORM_CATEGORIES,
  getEcommercePlatformCategoryCount,
  filterEcommercePlatformTools,
} from "@/lib/ecommerce-platforms";
import { ECOMMERCE_PLATFORM_EDITORIAL } from "@/data/ecommerce-platforms";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/ecommerce-platforms" } };
  }
  return {
    title: "Best E-Commerce Platforms (2026) — 20+ Platforms Reviewed",
    description: "Compare the best e-commerce platforms for online stores. Reviews of Shopify, BigCommerce, WooCommerce, Squarespace, and more.",
    keywords: ["ecommerce platform","best ecommerce platform 2026","online store builder","ecommerce software","headless commerce"],
    alternates: { canonical: "https://www.finderslist.com/ecommerce-platforms" },
  };
}

export default async function EcommercePlatformPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedEcommercePlatformTools();
  const allTools = getAllEcommercePlatformTools();
  const categories = getAllEcommercePlatformCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterEcommercePlatformTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "E-Commerce Platforms", item: "https://www.finderslist.com/ecommerce-platforms" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/ecommerce-platforms/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ECOMMERCE_PLATFORM_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-1.5 text-sm text-orange-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated e-commerce platforms
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of e-commerce platforms. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/ecommerce-platforms" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={ECOMMERCE_PLATFORM_EDITORIAL.title}
            intro={ECOMMERCE_PLATFORM_EDITORIAL.intro}
            buyerGuide={ECOMMERCE_PLATFORM_EDITORIAL.buyerGuide}
            faq={ECOMMERCE_PLATFORM_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ecommerce-platforms" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ECOMMERCE_PLATFORM_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/ecommerce-platforms"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/ecommerce-platforms" className="text-orange-400 hover:text-orange-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getEcommercePlatformCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/ecommerce-platforms/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/ecommerce-platforms" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="ecommerce-platforms" />
      </div>
    </div>
  );
}
