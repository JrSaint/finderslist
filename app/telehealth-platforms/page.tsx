...
import {
  getFeaturedTelehealthTools,
  getAllTelehealthTools,
  getAllTelehealthCategories,
  TELEHEALTH_CATEGORIES,
  getTelehealthCategoryCount,
  filterTelehealthTools,
} from "@/lib/telehealth-platforms";
import { TELEHEALTH_EDITORIAL } from "@/data/telehealth-platforms";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/telehealth-platforms" } };
  }
  return {
    title: "Best Telehealth Platforms (2026) — 15+ Services Compared",
    description: "Compare the best telehealth platforms for virtual doctor visits, mental health, and chronic care. Reviews of Teladoc, Amwell, MDLive, and more.",
    keywords: ["telehealth platforms","best telehealth 2026","virtual doctor visits","online doctor","telemedicine"],
    alternates: { canonical: "https://www.finderslist.com/telehealth-platforms" },
  };
}

export default async function TelehealthPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTelehealthTools();
  const allTools = getAllTelehealthTools();
  const categories = getAllTelehealthCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterTelehealthTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/telehealth-platforms/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TELEHEALTH_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-4 py-1.5 text-sm text-red-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated telehealth platforms
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of telehealth platforms. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/telehealth-platforms" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={TELEHEALTH_EDITORIAL.title}
            intro={TELEHEALTH_EDITORIAL.intro}
            buyerGuide={TELEHEALTH_EDITORIAL.buyerGuide}
            faq={TELEHEALTH_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/telehealth-platforms" />
          <ComparisonTable
            tools={featured as never[]}
            categories={TELEHEALTH_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/telehealth-platforms"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/telehealth-platforms" className="text-red-400 hover:text-red-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/telehealth-platforms" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = TELEHEALTH_CATEGORIES[cat];
                  const count = getTelehealthCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/telehealth-platforms/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/telehealth-platforms" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="telehealth-platforms" />
      </div>
    </div>
  );
}
