...
import type { TelehealthCategory } from "@/lib/telehealth-platforms";
...
export async function generateStaticParams() {
  return getAllTelehealthCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = TELEHEALTH_CATEGORIES[slug as TelehealthCategory];
  if (!category) return {};
  const tools = getTelehealthToolsByCategory(slug as TelehealthCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "telehealth platforms", category.label],
    alternates: { canonical: `https://www.finderslist.com/telehealth-platforms/category/${slug}` },
  };
}

export default async function TelehealthCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!TELEHEALTH_CATEGORIES[slug as TelehealthCategory]) notFound();

  const category = TELEHEALTH_CATEGORIES[slug as TelehealthCategory];
  const tools = getTelehealthToolsByCategory(slug as TelehealthCategory);
  const allCategories = getAllTelehealthCategories();
  const totalTools = getAllTelehealthTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/telehealth-platforms/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/telehealth-platforms" className="block w-full rounded-lg bg-red-600 hover:bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
