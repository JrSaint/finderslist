...
import type { CarAccidentLawyerCategory } from "@/lib/car-accident-lawyers";
...
export async function generateStaticParams() {
  return getAllCarAccidentLawyerCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = CAR_ACCIDENT_LAWYER_CATEGORIES[slug as CarAccidentLawyerCategory];
  if (!category) return {};
  const tools = getCarAccidentLawyerToolsByCategory(slug as CarAccidentLawyerCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "car accident lawyers", category.label],
    alternates: { canonical: `https://www.finderslist.com/car-accident-lawyers/category/${slug}` },
  };
}

export default async function CarAccidentLawyerCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!CAR_ACCIDENT_LAWYER_CATEGORIES[slug as CarAccidentLawyerCategory]) notFound();

  const category = CAR_ACCIDENT_LAWYER_CATEGORIES[slug as CarAccidentLawyerCategory];
  const tools = getCarAccidentLawyerToolsByCategory(slug as CarAccidentLawyerCategory);
  const allCategories = getAllCarAccidentLawyerCategories();
  const totalTools = getAllCarAccidentLawyerTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/car-accident-lawyers/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/car-accident-lawyers/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/car-accident-lawyers" className="block w-full rounded-lg bg-red-600 hover:bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
