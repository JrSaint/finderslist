...
import type { OnlineDegreeCategory } from "@/lib/online-degree-programs";
...
export async function generateStaticParams() {
  return getAllOnlineDegreeCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ONLINE_DEGREE_CATEGORIES[slug as OnlineDegreeCategory];
  if (!category) return {};
  const tools = getOnlineDegreeToolsByCategory(slug as OnlineDegreeCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "online degree programs", category.label],
    alternates: { canonical: `https://www.finderslist.com/online-degree-programs/category/${slug}` },
  };
}

export default async function OnlineDegreeCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ONLINE_DEGREE_CATEGORIES[slug as OnlineDegreeCategory]) notFound();

  const category = ONLINE_DEGREE_CATEGORIES[slug as OnlineDegreeCategory];
  const tools = getOnlineDegreeToolsByCategory(slug as OnlineDegreeCategory);
  const allCategories = getAllOnlineDegreeCategories();
  const totalTools = getAllOnlineDegreeTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/online-degree-programs/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/online-degree-programs" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
