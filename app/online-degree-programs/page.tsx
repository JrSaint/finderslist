...
import {
  getFeaturedOnlineDegreeTools,
  getAllOnlineDegreeTools,
  getAllOnlineDegreeCategories,
  ONLINE_DEGREE_CATEGORIES,
  getOnlineDegreeCategoryCount,
  filterOnlineDegreeTools,
} from "@/lib/online-degree-programs";
import { ONLINE_DEGREE_EDITORIAL } from "@/data/online-degree-programs";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/online-degree-programs" } };
  }
  return {
    title: "Best Online Degree Programs (2026) — 15+ Universities Compared",
    description: "Compare the best online degree programs from accredited universities. Reviews of programs from WGU, SNHU, ASU Online, Purdue Global, and more.",
    keywords: ["online degree programs","best online colleges 2026","online bachelors degree","online masters degree","accredited online programs"],
    alternates: { canonical: "https://www.finderslist.com/online-degree-programs" },
  };
}

export default async function OnlineDegreePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedOnlineDegreeTools();
  const allTools = getAllOnlineDegreeTools();
  const categories = getAllOnlineDegreeCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterOnlineDegreeTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/online-degree-programs/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ONLINE_DEGREE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated online degree programs
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of online degree programs. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/online-degree-programs" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/online-degree-programs"
        ...
        {!isFiltering && (
          <EditorialSection
            title={ONLINE_DEGREE_EDITORIAL.title}
            intro={ONLINE_DEGREE_EDITORIAL.intro}
            buyerGuide={ONLINE_DEGREE_EDITORIAL.buyerGuide}
            faq={ONLINE_DEGREE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/online-degree-programs" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ONLINE_DEGREE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/online-degree-programs"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/online-degree-programs" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/online-degree-programs" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ONLINE_DEGREE_CATEGORIES[cat];
                  const count = getOnlineDegreeCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/online-degree-programs/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/online-degree-programs" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="online-degree-programs" />
      </div>
    </div>
  );
}
