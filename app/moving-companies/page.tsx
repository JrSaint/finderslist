...
import {
  getFeaturedMovingCompanyTools,
  getAllMovingCompanyTools,
  getAllMovingCompanyCategories,
  MOVING_COMPANY_CATEGORIES,
  getMovingCompanyCategoryCount,
  filterMovingCompanyTools,
} from "@/lib/moving-companies";
import { MOVING_COMPANY_EDITORIAL } from "@/data/moving-companies";
...
export default async function MovingCompanyPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedMovingCompanyTools();
  const allTools = getAllMovingCompanyTools();
  const categories = getAllMovingCompanyCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterMovingCompanyTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/moving-companies/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-1.5 text-sm text-orange-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated moving companies
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of moving companies. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/moving-companies" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={MOVING_COMPANY_EDITORIAL.title}
            intro={MOVING_COMPANY_EDITORIAL.intro}
            buyerGuide={MOVING_COMPANY_EDITORIAL.buyerGuide}
            faq={MOVING_COMPANY_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/moving-companies" />
          <ComparisonTable
            tools={featured as never[]}
            categories={MOVING_COMPANY_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/moving-companies"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/moving-companies" className="text-orange-400 hover:text-orange-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/moving-companies" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = MOVING_COMPANY_CATEGORIES[cat];
                  const count = getMovingCompanyCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/moving-companies/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/moving-companies" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="moving-companies" />
      </div>
    </div>
  );
}
