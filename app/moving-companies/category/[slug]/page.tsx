...
import type { MovingCompanyCategory } from "@/lib/moving-companies";
...
export async function generateStaticParams() {
  return getAllMovingCompanyCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = MOVING_COMPANY_CATEGORIES[slug as MovingCompanyCategory];
  if (!category) return {};
  const tools = getMovingCompanyToolsByCategory(slug as MovingCompanyCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "moving companies", category.label],
    alternates: { canonical: `https://www.finderslist.com/moving-companies/category/${slug}` },
  };
}

export default async function MovingCompanyCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!MOVING_COMPANY_CATEGORIES[slug as MovingCompanyCategory]) notFound();

  const category = MOVING_COMPANY_CATEGORIES[slug as MovingCompanyCategory];
  const tools = getMovingCompanyToolsByCategory(slug as MovingCompanyCategory);
  const allCategories = getAllMovingCompanyCategories();
  const totalTools = getAllMovingCompanyTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/moving-companies/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/moving-companies" className="block w-full rounded-lg bg-orange-600 hover:bg-orange-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
