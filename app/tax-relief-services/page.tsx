...
import {
  getFeaturedTaxReliefTools,
  getAllTaxReliefTools,
  getAllTaxReliefCategories,
  TAX_RELIEF_CATEGORIES,
  getTaxReliefCategoryCount,
  filterTaxReliefTools,
} from "@/lib/tax-relief-services";
import { TAX_RELIEF_EDITORIAL } from "@/data/tax-relief-services";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/tax-relief-services" } };
  }
  return {
    title: "Best Tax Relief Services (2026) — 15+ Companies Compared",
    ...
    keywords: ["tax relief", "best tax relief companies", "IRS tax debt help", "offer in compromise", "tax resolution services", "tax settlement"],
    alternates: { canonical: "https://www.finderslist.com/tax-relief-services" },
  };
}

export default async function TaxReliefServicesPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTaxReliefTools();
  const allTools = getAllTaxReliefTools();
  const categories = getAllTaxReliefCategories();
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TAX_RELIEF_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-4 py-1.5 text-sm text-red-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated tax relief services
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of tax relief services. Compare top companies with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/tax-relief-services" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={TAX_RELIEF_EDITORIAL.title}
            intro={TAX_RELIEF_EDITORIAL.intro}
            buyerGuide={TAX_RELIEF_EDITORIAL.buyerGuide}
            faq={TAX_RELIEF_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/tax-relief-services" />
          <ComparisonTable
            tools={featured as never[]}
            categories={TAX_RELIEF_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/tax-relief-services"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/tax-relief-services" className="text-red-400 hover:text-red-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getTaxReliefCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/tax-relief-services/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/tax-relief-services" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="tax-relief-services" />
      </div>
    </div>
  );
}
