...
import { ERP_EDITORIAL } from "@/data/erp-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/erp-software" } };
  }
  return {
    title: "Best ERP Software (2026) — 15+ Solutions Compared",
    description: "Compare the best ERP software for enterprise, manufacturing, and small business. Reviews of SAP S/4HANA, Oracle NetSuite, Microsoft Dynamics 365, Acumatica, and more.",
    keywords: ["ERP software", "best ERP software 2026", "enterprise resource planning", "ERP solutions", "cloud ERP"],
    alternates: { canonical: "https://www.finderslist.com/erp-software" },
  };
}

export default async function ERPSoftwarePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedERPTools();
  const allTools = getAllERPTools();
  const categories = getAllERPCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterERPTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/erp-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ERP_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated ERP software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of ERP software. Compare top solutions with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/erp-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={ERP_EDITORIAL.title}
            intro={ERP_EDITORIAL.intro}
            buyerGuide={ERP_EDITORIAL.buyerGuide}
            faq={ERP_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/erp-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ERP_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/erp-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/erp-software" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/erp-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ERP_CATEGORIES[cat];
                  const count = getERPCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/erp-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/erp-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="erp-software" />
      </div>
    </div>
  );
}
