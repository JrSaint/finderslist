...
import type { ERPCategory } from "@/lib/erp-software";
...
export async function generateStaticParams() {
  return getAllERPCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ERP_CATEGORIES[slug as ERPCategory];
  if (!category) return {};
  const tools = getERPToolsByCategory(slug as ERPCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "ERP software", category.label],
    alternates: { canonical: `https://www.finderslist.com/erp-software/category/${slug}` },
  };
}

export default async function ERPCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ERP_CATEGORIES[slug as ERPCategory]) notFound();

  const category = ERP_CATEGORIES[slug as ERPCategory];
  const tools = getERPToolsByCategory(slug as ERPCategory);
  const allCategories = getAllERPCategories();
  const totalTools = getAllERPTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/erp-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/erp-software" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
