...
import {
  getFeaturedSupplyChainTools,
  getAllSupplyChainTools,
  getAllSupplyChainCategories,
  SUPPLY_CHAIN_CATEGORIES,
  getSupplyChainCategoryCount,
  filterSupplyChainTools,
} from "@/lib/supply-chain-software";
import { SUPPLY_CHAIN_EDITORIAL } from "@/data/supply-chain-software";
...
export default async function SupplyChainPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSupplyChainTools();
  const allTools = getAllSupplyChainTools();
  const categories = getAllSupplyChainCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSupplyChainTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/supply-chain-software/tools/${tool.slug}`,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-4 py-1.5 text-sm text-blue-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated supply chain software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of supply chain software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/supply-chain-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/supply-chain-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={SUPPLY_CHAIN_EDITORIAL.title}
            intro={SUPPLY_CHAIN_EDITORIAL.intro}
            buyerGuide={SUPPLY_CHAIN_EDITORIAL.buyerGuide}
            faq={SUPPLY_CHAIN_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/supply-chain-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={SUPPLY_CHAIN_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/supply-chain-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/supply-chain-software" className="text-blue-400 hover:text-blue-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getSupplyChainCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/supply-chain-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/supply-chain-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="supply-chain-software" />
      </div>
    </div>
  );
}
