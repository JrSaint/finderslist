...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/crm-tools" } };
  }
  return {
    title: "Best CRM & Sales Tools Directory (2026) — 35+ Tools Reviewed",
    description:
      "Discover the best CRM and sales tools for sales teams, SDRs, and revenue operations. Compare HubSpot, Salesforce, Pipedrive, Apollo.io, Gong, PandaDoc, and 30+ more with honest reviews, pricing, and real-world use cases.",
    keywords: [
      "crm software",
      "best crm tools 2026",
    ...
    alternates: { canonical: "https://www.finderslist.com/crm-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(CRM_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function CRMToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCRMTools();
  const allTools = getAllCRMTools();
  const categories = getAllCRMCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCRMTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "CRM & Sales Tools", item: "https://www.finderslist.com/crm-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    description: "Discover the best CRM and sales tools for sales teams, SDRs, and revenue operations. Compare HubSpot, Salesforce, Pipedrive, Apollo.io, Gong, PandaDoc, and 30+ more with honest reviews, pricing, and real-world use cases.",
    numberOfItems: allTools.length,
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/crm-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (CRM_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_80%_60%,rgba(14,165,233,0.06),transparent)] pointer-events-none" />
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-4 py-1.5 text-sm text-sky-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated CRM & sales tools
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/crm-tools" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/crm-tools" className="text-sky-400 hover:text-sky-300 text-sm font-medium">
                  Clear all filters →
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/crm-tools" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = CRM_CATEGORIES[cat];
                  const count = getCRMCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/crm-tools/category/${cat}`}
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-white group-hover:text-sky-300 transition-colors leading-tight">
                        {info.label}
                      </span>
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/crm-tools" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="crm-tools" />
      </div>
    </div>
  );
}
