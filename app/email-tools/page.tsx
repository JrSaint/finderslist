...
import {
  getFeaturedEmailTools,
  getAllEmailTools,
  getAllEmailCategories,
  EMAIL_CATEGORIES,
  getEmailCategoryCount,
  filterEmailTools,
  EMAIL_ROLES,
} from "@/lib/email-tools";
import { EMAIL_EDITORIAL } from "@/data/email-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/email-tools" } };
  }
  return {
    title: "Best Email Marketing Tools Directory (2026) — 30+ Tools Reviewed",
    description: "The most comprehensive directory of email marketing tools. Compare Klaviyo, Mailchimp, ConvertKit, ActiveCampaign, Brevo, and 30+ more with honest reviews.",
    alternates: { canonical: "https://www.finderslist.com/email-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(EMAIL_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function EmailToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedEmailTools();
  const allTools = getAllEmailTools();
  const categories = getAllEmailCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterEmailTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Email Marketing Tools", item: "https://www.finderslist.com/email-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/email-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (EMAIL_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={EMAIL_EDITORIAL.title}
            intro={EMAIL_EDITORIAL.intro}
            buyerGuide={EMAIL_EDITORIAL.buyerGuide}
            faq={EMAIL_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/email-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={EMAIL_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/email-tools"
          />
          </>
        )}
        ...
        {isFiltering ? (
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/email-tools" className="text-red-400 hover:text-red-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/email-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getEmailCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/email-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-red-500/30 transition-all group">
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {featured.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/email-tools" />))}
              </div>
            </section>
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/email-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="email-tools" />
      </div>
    </div>
  );
}
