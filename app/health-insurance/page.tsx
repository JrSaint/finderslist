...
import {
  getFeaturedHealthInsuranceTools,
  getAllHealthInsuranceTools,
  getAllHealthInsuranceCategories,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/health-insurance" } };
  }
  return {
    title: "Best Health Insurance Plans (2026) — 15+ Options Compared",
    description: "Compare the best health insurance plans and marketplaces. Reviews of Blue Cross Blue Shield, UnitedHealthcare, Kaiser Permanente, Oscar, and more.",
    keywords: ["health insurance","best health insurance 2026","health insurance marketplace","affordable health plans","health insurance comparison"],
    alternates: { canonical: "https://www.finderslist.com/health-insurance" },
  };
}

export default async function HealthInsurancePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedHealthInsuranceTools();
  const allTools = getAllHealthInsuranceTools();
  const categories = getAllHealthInsuranceCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterHealthInsuranceTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Health Insurance Plans", item: "https://www.finderslist.com/health-insurance" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/health-insurance/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (HEALTH_INSURANCE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-4 py-1.5 text-sm text-red-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated health insurance
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of health insurance. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/health-insurance" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/health-insurance" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/health-insurance" className="text-red-400 hover:text-red-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/health-insurance" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = HEALTH_INSURANCE_CATEGORIES[cat];
                  const count = getHealthInsuranceCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/health-insurance/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/health-insurance" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="health-insurance" />
      </div>
    </div>
  );
}
