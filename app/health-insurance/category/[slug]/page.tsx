...
import type { HealthInsuranceCategory } from "@/lib/health-insurance";
...
export async function generateStaticParams() {
  return getAllHealthInsuranceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = HEALTH_INSURANCE_CATEGORIES[slug as HealthInsuranceCategory];
  if (!category) return {};
  const tools = getHealthInsuranceToolsByCategory(slug as HealthInsuranceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "health insurance", category.label],
    alternates: { canonical: `https://www.finderslist.com/health-insurance/category/${slug}` },
  };
}

export default async function HealthInsuranceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!HEALTH_INSURANCE_CATEGORIES[slug as HealthInsuranceCategory]) notFound();

  const category = HEALTH_INSURANCE_CATEGORIES[slug as HealthInsuranceCategory];
  const tools = getHealthInsuranceToolsByCategory(slug as HealthInsuranceCategory);
  const allCategories = getAllHealthInsuranceCategories();
  const totalTools = getAllHealthInsuranceTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/health-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/health-insurance" className="block w-full rounded-lg bg-red-600 hover:bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
