...
import {
  getFeaturedEndpointSecurityTools,
  getAllEndpointSecurityTools,
  getAllEndpointSecurityCategories,
  ENDPOINT_SECURITY_CATEGORIES,
  getEndpointSecurityCategoryCount,
  filterEndpointSecurityTools,
} from "@/lib/endpoint-security-software";
import { ENDPOINT_SECURITY_EDITORIAL } from "@/data/endpoint-security-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/endpoint-security-software" } };
  }
  return {
    title: "Best Endpoint Security Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best endpoint security and EDR platforms. Reviews of CrowdStrike, SentinelOne, Microsoft Defender, Bitdefender, and more.",
    keywords: ["endpoint security software","edr software 2026","endpoint protection","antivirus business","endpoint detection response"],
    alternates: { canonical: "https://www.finderslist.com/endpoint-security-software" },
  };
}

export default async function EndpointSecurityPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedEndpointSecurityTools();
  const allTools = getAllEndpointSecurityTools();
  const categories = getAllEndpointSecurityCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterEndpointSecurityTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/endpoint-security-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ENDPOINT_SECURITY_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-4 py-1.5 text-sm text-red-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated endpoint security software
          </div>
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/endpoint-security-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/endpoint-security-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={ENDPOINT_SECURITY_EDITORIAL.title}
            intro={ENDPOINT_SECURITY_EDITORIAL.intro}
            buyerGuide={ENDPOINT_SECURITY_EDITORIAL.buyerGuide}
            faq={ENDPOINT_SECURITY_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/endpoint-security-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ENDPOINT_SECURITY_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/endpoint-security-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/endpoint-security-software" className="text-red-400 hover:text-red-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/endpoint-security-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ENDPOINT_SECURITY_CATEGORIES[cat];
                  const count = getEndpointSecurityCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/endpoint-security-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/endpoint-security-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="endpoint-security-software" />
      </div>
    </div>
  );
}
