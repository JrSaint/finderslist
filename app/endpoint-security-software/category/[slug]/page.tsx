...
import type { EndpointSecurityCategory } from "@/lib/endpoint-security-software";
...
export async function generateStaticParams() {
  return getAllEndpointSecurityCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ENDPOINT_SECURITY_CATEGORIES[slug as EndpointSecurityCategory];
  if (!category) return {};
  const tools = getEndpointSecurityToolsByCategory(slug as EndpointSecurityCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "endpoint security software", category.label],
    alternates: { canonical: `https://www.finderslist.com/endpoint-security-software/category/${slug}` },
  };
}

export default async function EndpointSecurityCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ENDPOINT_SECURITY_CATEGORIES[slug as EndpointSecurityCategory]) notFound();

  const category = ENDPOINT_SECURITY_CATEGORIES[slug as EndpointSecurityCategory];
  const tools = getEndpointSecurityToolsByCategory(slug as EndpointSecurityCategory);
  const allCategories = getAllEndpointSecurityCategories();
  const totalTools = getAllEndpointSecurityTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/endpoint-security-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/endpoint-security-software" className="block w-full rounded-lg bg-red-600 hover:bg-red-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
