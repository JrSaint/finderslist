...
import {
  getFeaturedPestControlTools,
  getAllPestControlTools,
  getAllPestControlCategories,
  PEST_CONTROL_CATEGORIES,
  getPestControlCategoryCount,
  filterPestControlTools,
} from "@/lib/pest-control-services";
import { PEST_CONTROL_EDITORIAL } from "@/data/pest-control-services";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/pest-control-services" } };
  }
  return {
    title: "Best Pest Control Companies (2026) — 15+ Services Compared",
    description: "Compare the best pest control companies for termites, bed bugs, and general pest prevention. Reviews of Terminix, Orkin, Aptive, and more.",
    keywords: ["pest control services","best pest control 2026","termite treatment","exterminator","pest prevention"],
    alternates: { canonical: "https://www.finderslist.com/pest-control-services" },
  };
}

export default async function PestControlPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedPestControlTools();
  const allTools = getAllPestControlTools();
  const categories = getAllPestControlCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterPestControlTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Pest Control Companies", item: "https://www.finderslist.com/pest-control-services" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/pest-control-services/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (PEST_CONTROL_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated pest control services
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of pest control services. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/pest-control-services" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/pest-control-services"
        ...
        {!isFiltering && (
          <EditorialSection
            title={PEST_CONTROL_EDITORIAL.title}
            intro={PEST_CONTROL_EDITORIAL.intro}
            buyerGuide={PEST_CONTROL_EDITORIAL.buyerGuide}
            faq={PEST_CONTROL_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/pest-control-services" />
          <ComparisonTable
            tools={featured as never[]}
            categories={PEST_CONTROL_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/pest-control-services"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/pest-control-services" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/pest-control-services" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = PEST_CONTROL_CATEGORIES[cat];
                  const count = getPestControlCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/pest-control-services/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/pest-control-services" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="pest-control-services" />
      </div>
    </div>
  );
}
