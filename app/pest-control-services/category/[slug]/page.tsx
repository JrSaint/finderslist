...
import type { PestControlCategory } from "@/lib/pest-control-services";
...
export async function generateStaticParams() {
  return getAllPestControlCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = PEST_CONTROL_CATEGORIES[slug as PestControlCategory];
  if (!category) return {};
  const tools = getPestControlToolsByCategory(slug as PestControlCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "pest control services", category.label],
    alternates: { canonical: `https://www.finderslist.com/pest-control-services/category/${slug}` },
  };
}
...
export default async function PestControlCategoryPage({ params }: Props) {
  ...
  if (!PEST_CONTROL_CATEGORIES[slug as PestControlCategory]) notFound();

  const category = PEST_CONTROL_CATEGORIES[slug as PestControlCategory];
  const tools = getPestControlToolsByCategory(slug as PestControlCategory);
  const allCategories = getAllPestControlCategories();
  const totalTools = getAllPestControlTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/pest-control-services/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/pest-control-services" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
