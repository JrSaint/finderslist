...
import {
  getFeaturedBudgetingAppTools,
  getAllBudgetingAppTools,
  getAllBudgetingAppCategories,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/budgeting-apps" } };
  }
  return {
    title: "Best Budgeting Apps (2026) — 15+ Apps Reviewed & Compared",
    description: "Compare the best budgeting apps for personal finance, envelope budgeting, and expense tracking. Reviews of YNAB, Mint, Monarch Money, and more.",
    keywords: ["budgeting apps","best budget apps 2026","personal finance apps","expense tracking","money management"],
    alternates: { canonical: "https://www.finderslist.com/budgeting-apps" },
  };
}

export default async function BudgetingAppPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedBudgetingAppTools();
  const allTools = getAllBudgetingAppTools();
  const categories = getAllBudgetingAppCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterBudgetingAppTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/budgeting-apps/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (BUDGETING_APP_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 text-sm text-green-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated budgeting apps
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of budgeting apps. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/budgeting-apps" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/budgeting-apps" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/budgeting-apps" className="text-green-400 hover:text-green-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/budgeting-apps" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = BUDGETING_APP_CATEGORIES[cat];
                  const count = getBudgetingAppCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/budgeting-apps/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/budgeting-apps" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="budgeting-apps" />
      </div>
    </div>
  );
}
