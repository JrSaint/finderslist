...
import type { BusinessVPNCategory } from "@/lib/business-vpn-software";
...
export async function generateStaticParams() {
  return getAllBusinessVPNCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = BUSINESS_VPN_CATEGORIES[slug as BusinessVPNCategory];
  if (!category) return {};
  const tools = getBusinessVPNToolsByCategory(slug as BusinessVPNCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "business vpn software", category.label],
    alternates: { canonical: `https://www.finderslist.com/business-vpn-software/category/${slug}` },
  };
}

export default async function BusinessVPNCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!BUSINESS_VPN_CATEGORIES[slug as BusinessVPNCategory]) notFound();

  const category = BUSINESS_VPN_CATEGORIES[slug as BusinessVPNCategory];
  const tools = getBusinessVPNToolsByCategory(slug as BusinessVPNCategory);
  const allCategories = getAllBusinessVPNCategories();
  const totalTools = getAllBusinessVPNTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/business-vpn-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/business-vpn-software" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
