...
import {
  getFeaturedSupportTools,
  getAllSupportTools,
  getAllSupportCategories,
  SUPPORT_CATEGORIES,
  getSupportCategoryCount,
  filterSupportTools,
  SUPPORT_ROLES,
...
const roles = Object.fromEntries(
  Object.entries(SUPPORT_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function SupportToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSupportTools();
  const allTools = getAllSupportTools();
  const categories = getAllSupportCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSupportTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Customer Support & CX Tools", item: "https://www.finderslist.com/support-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Customer Support & CX Tools",
    description: "The most comprehensive directory of customer support and CX tools. Compare Zendesk, Intercom, Freshdesk, Help Scout, Gorgias, and 30+ more.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/support-tools/tools/${tool.slug}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/support-tools" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/support-tools" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/support-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = SUPPORT_CATEGORIES[cat];
                  const count = getSupportCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/support-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-indigo-500/30 transition-all group">
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/support-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="support-tools" />
      </div>
    </div>
  );
}
