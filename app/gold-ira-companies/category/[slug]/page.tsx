...
import type { GoldIRACategory } from "@/lib/gold-ira-companies";
...
export async function generateStaticParams() {
  return getAllGoldIRACategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = GOLD_IRA_CATEGORIES[slug as GoldIRACategory];
  if (!category) return {};
  ...
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/gold-ira-companies/category/${slug}` },
  };
}

export default async function GoldIRACategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!GOLD_IRA_CATEGORIES[slug as GoldIRACategory]) notFound();

  const category = GOLD_IRA_CATEGORIES[slug as GoldIRACategory];
  const tools = getGoldIRAToolsByCategory(slug as GoldIRACategory);
  const allCategories = getAllGoldIRACategories();
  const totalTools = getAllGoldIRATools().length;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `Best ${category.label} Companies`,
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/gold-ira-companies/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/gold-ira-companies/category/${cat}`}
                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
                      isActive ? "bg-yellow-500/20 text-yellow-300 border border-yellow-500/30" : "text-slate-400 hover:text-white hover:bg-white/5"
                    }`}>
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/gold-ira-companies" className="block w-full rounded-lg bg-yellow-600 hover:bg-yellow-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
