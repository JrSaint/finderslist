...
import { GOLD_IRA_EDITORIAL } from "@/data/gold-ira-companies";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/gold-ira-companies" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/gold-ira-companies" },
  };
}
...
export default async function GoldIRACompaniesPage({ searchParams }: Props) {
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (GOLD_IRA_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={GOLD_IRA_EDITORIAL.title}
            intro={GOLD_IRA_EDITORIAL.intro}
            buyerGuide={GOLD_IRA_EDITORIAL.buyerGuide}
            faq={GOLD_IRA_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/gold-ira-companies" />
          <ComparisonTable
            tools={featured as never[]}
            categories={GOLD_IRA_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/gold-ira-companies"
          />
          </>
        )}
        ...
        <RelatedDirectories currentSlug="gold-ira-companies" />
      </div>
    </div>
  );
}
