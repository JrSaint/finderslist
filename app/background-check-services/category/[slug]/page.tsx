...
import type { BackgroundCheckCategory } from "@/lib/background-check-services";
...
export async function generateStaticParams() {
  return getAllBackgroundCheckCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = BACKGROUND_CHECK_CATEGORIES[slug as BackgroundCheckCategory];
  if (!category) return {};
  const tools = getBackgroundCheckToolsByCategory(slug as BackgroundCheckCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "background check services", category.label],
    alternates: { canonical: `https://www.finderslist.com/background-check-services/category/${slug}` },
  };
}

export default async function BackgroundCheckServicesCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!BACKGROUND_CHECK_CATEGORIES[slug as BackgroundCheckCategory]) notFound();

  const category = BACKGROUND_CHECK_CATEGORIES[slug as BackgroundCheckCategory];
  const tools = getBackgroundCheckToolsByCategory(slug as BackgroundCheckCategory);
  const allCategories = getAllBackgroundCheckCategories();
  const totalTools = getAllBackgroundCheckTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/background-check-services/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/background-check-services" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
