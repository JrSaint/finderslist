...
import type { MortgageLendingCategory } from "@/lib/mortgage-lending-software";
...
export async function generateStaticParams() {
  return getAllMortgageLendingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = MORTGAGE_LENDING_CATEGORIES[slug as MortgageLendingCategory];
  if (!category) return {};
  const tools = getMortgageLendingToolsByCategory(slug as MortgageLendingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "mortgage & lending software", category.label],
    alternates: { canonical: `https://www.finderslist.com/mortgage-lending-software/category/${slug}` },
  };
}

export default async function MortgageLendingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!MORTGAGE_LENDING_CATEGORIES[slug as MortgageLendingCategory]) notFound();

  const category = MORTGAGE_LENDING_CATEGORIES[slug as MortgageLendingCategory];
  const tools = getMortgageLendingToolsByCategory(slug as MortgageLendingCategory);
  const allCategories = getAllMortgageLendingCategories();
  const totalTools = getAllMortgageLendingTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/mortgage-lending-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/mortgage-lending-software" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
