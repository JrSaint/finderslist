...
import type { SolarPanelCategory } from "@/lib/solar-panel-companies";
...
export async function generateStaticParams() {
  return getAllSolarPanelCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = SOLAR_PANEL_CATEGORIES[slug as SolarPanelCategory];
  if (!category) return {};
  const tools = getSolarPanelToolsByCategory(slug as SolarPanelCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "solar panel companies", category.label],
    alternates: { canonical: `https://www.finderslist.com/solar-panel-companies/category/${slug}` },
  };
}

export default async function SolarPanelCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!SOLAR_PANEL_CATEGORIES[slug as SolarPanelCategory]) notFound();

  const category = SOLAR_PANEL_CATEGORIES[slug as SolarPanelCategory];
  const tools = getSolarPanelToolsByCategory(slug as SolarPanelCategory);
  const allCategories = getAllSolarPanelCategories();
  const totalTools = getAllSolarPanelTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/solar-panel-companies/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/solar-panel-companies" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
