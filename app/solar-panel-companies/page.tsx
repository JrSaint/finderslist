...
import {
  getFeaturedSolarPanelTools,
  getAllSolarPanelTools,
  getAllSolarPanelCategories,
  SOLAR_PANEL_CATEGORIES,
  getSolarPanelCategoryCount,
  filterSolarPanelTools,
} from "@/lib/solar-panel-companies";
import { SOLAR_PANEL_EDITORIAL } from "@/data/solar-panel-companies";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/solar-panel-companies" } };
  }
  return {
    title: "Best Solar Panel Companies (2026) — 15+ Installers Compared",
    description: "Compare the best solar panel companies and installers. Reviews of SunPower, Tesla Solar, Sunrun, Palmetto, and more.",
    keywords: ["solar panel companies","best solar installers 2026","home solar panels","solar energy","solar installation"],
    alternates: { canonical: "https://www.finderslist.com/solar-panel-companies" },
  };
}

export default async function SolarPanelPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSolarPanelTools();
  const allTools = getAllSolarPanelTools();
  const categories = getAllSolarPanelCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSolarPanelTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Solar Panel Companies", item: "https://www.finderslist.com/solar-panel-companies" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/solar-panel-companies/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (SOLAR_PANEL_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated solar panel companies
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of solar panel companies. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/solar-panel-companies" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/solar-panel-companies"
        ...
        {!isFiltering && (
          <EditorialSection
            title={SOLAR_PANEL_EDITORIAL.title}
            intro={SOLAR_PANEL_EDITORIAL.intro}
            buyerGuide={SOLAR_PANEL_EDITORIAL.buyerGuide}
            faq={SOLAR_PANEL_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/solar-panel-companies" />
          <ComparisonTable
            tools={featured as never[]}
            categories={SOLAR_PANEL_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/solar-panel-companies"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/solar-panel-companies" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/solar-panel-companies" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = SOLAR_PANEL_CATEGORIES[cat];
                  const count = getSolarPanelCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/solar-panel-companies/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/solar-panel-companies" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="solar-panel-companies" />
      </div>
    </div>
  );
}
