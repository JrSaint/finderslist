...
import {
  getFeaturedLiveChatTools,
  getAllLiveChatTools,
  getAllLiveChatCategories,
  LIVE_CHAT_CATEGORIES,
  getLiveChatCategoryCount,
  filterLiveChatTools,
} from "@/lib/live-chat-software";
import { LIVE_CHAT_EDITORIAL } from "@/data/live-chat-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/live-chat-software" } };
  }
  return {
    title: "Best Live Chat Software (2026) — Support, Sales & AI Chat Compared",
    description: "Compare the best live chat software in 2026 for customer support, sales, and AI-powered conversations. Reviews of Intercom, LiveChat, Tidio, Crisp, and more.",
    keywords: ["live chat software","best live chat 2026","customer support chat","ai chatbot platform","website chat widget"],
    alternates: { canonical: "https://www.finderslist.com/live-chat-software" },
  };
}

export default async function LiveChatPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedLiveChatTools();
  const allTools = getAllLiveChatTools();
  const categories = getAllLiveChatCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterLiveChatTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/live-chat-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (LIVE_CHAT_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated live chat software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of live chat software. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/live-chat-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={LIVE_CHAT_EDITORIAL.title}
            intro={LIVE_CHAT_EDITORIAL.intro}
            buyerGuide={LIVE_CHAT_EDITORIAL.buyerGuide}
            faq={LIVE_CHAT_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/live-chat-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={LIVE_CHAT_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/live-chat-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/live-chat-software" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/live-chat-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = LIVE_CHAT_CATEGORIES[cat];
                  const count = getLiveChatCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/live-chat-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/live-chat-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="live-chat-software" />
      </div>
    </div>
  );
}
