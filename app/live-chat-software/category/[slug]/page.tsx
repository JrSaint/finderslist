...
import type { LiveChatCategory } from "@/lib/live-chat-software";
...
export async function generateStaticParams() {
  return getAllLiveChatCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = LIVE_CHAT_CATEGORIES[slug as LiveChatCategory];
  if (!category) return {};
  ...
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/live-chat-software/category/${slug}` },
  };
}

export default async function LiveChatCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!LIVE_CHAT_CATEGORIES[slug as LiveChatCategory]) notFound();

  const category = LIVE_CHAT_CATEGORIES[slug as LiveChatCategory];
  const tools = getLiveChatToolsByCategory(slug as LiveChatCategory);
  const allCategories = getAllLiveChatCategories();
  const totalTools = getAllLiveChatTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/live-chat-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/live-chat-software" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
