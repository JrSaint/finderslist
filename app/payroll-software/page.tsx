...
import {
  getFeaturedPayrollTools,
  getAllPayrollTools,
  getAllPayrollCategories,
  PAYROLL_CATEGORIES,
  getPayrollCategoryCount,
  filterPayrollTools,
} from "@/lib/payroll-software";
import { PAYROLL_EDITORIAL } from "@/data/payroll-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/payroll-software" } };
  }
  return {
    title: "Best Payroll Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best payroll software for small business, enterprise, and global teams. Reviews of Gusto, ADP, Rippling, Deel, and 15+ more.",
    keywords: ["payroll software","best payroll software 2026","online payroll","payroll processing","global payroll"],
    alternates: { canonical: "https://www.finderslist.com/payroll-software" },
  };
}

export default async function PayrollPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedPayrollTools();
  const allTools = getAllPayrollTools();
  const categories = getAllPayrollCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterPayrollTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/payroll-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (PAYROLL_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated payroll software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of payroll software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/payroll-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={PAYROLL_EDITORIAL.title}
            intro={PAYROLL_EDITORIAL.intro}
            buyerGuide={PAYROLL_EDITORIAL.buyerGuide}
            faq={PAYROLL_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/payroll-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={PAYROLL_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/payroll-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/payroll-software" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/payroll-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = PAYROLL_CATEGORIES[cat];
                  const count = getPayrollCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/payroll-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/payroll-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="payroll-software" />
      </div>
    </div>
  );
}
