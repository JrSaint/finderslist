...
import type { PayrollCategory } from "@/lib/payroll-software";
...
export async function generateStaticParams() {
  return getAllPayrollCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = PAYROLL_CATEGORIES[slug as PayrollCategory];
  if (!category) return {};
  const tools = getPayrollToolsByCategory(slug as PayrollCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "payroll software", category.label],
    alternates: { canonical: `https://www.finderslist.com/payroll-software/category/${slug}` },
  };
}

export default async function PayrollCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!PAYROLL_CATEGORIES[slug as PayrollCategory]) notFound();

  const category = PAYROLL_CATEGORIES[slug as PayrollCategory];
  const tools = getPayrollToolsByCategory(slug as PayrollCategory);
  const allCategories = getAllPayrollCategories();
  const totalTools = getAllPayrollTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/payroll-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/payroll-software" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
