...
import {
  getFeaturedPersonalLoanTools,
  getAllPersonalLoanTools,
  getAllPersonalLoanCategories,
  PERSONAL_LOAN_CATEGORIES,
  getPersonalLoanCategoryCount,
  filterPersonalLoanTools,
} from "@/lib/personal-loans";
import { PERSONAL_LOAN_EDITORIAL } from "@/data/personal-loans";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/personal-loans" } };
  }
  return {
    title: "Best Personal Loans (2026) — 15+ Lenders Compared",
    description: "Compare the best personal loan lenders for debt consolidation, home improvement, and more. Reviews of SoFi, LightStream, Marcus, Prosper, and more.",
    keywords: ["personal loans","best personal loans 2026","debt consolidation loans","online personal loans","low interest personal loans"],
    alternates: { canonical: "https://www.finderslist.com/personal-loans" },
  };
}

export default async function PersonalLoanPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedPersonalLoanTools();
  const allTools = getAllPersonalLoanTools();
  const categories = getAllPersonalLoanCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterPersonalLoanTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/personal-loans/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (PERSONAL_LOAN_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated personal loans
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of personal loans. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/personal-loans" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={PERSONAL_LOAN_EDITORIAL.title}
            intro={PERSONAL_LOAN_EDITORIAL.intro}
            buyerGuide={PERSONAL_LOAN_EDITORIAL.buyerGuide}
            faq={PERSONAL_LOAN_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/personal-loans" />
          <ComparisonTable
            tools={featured as never[]}
            categories={PERSONAL_LOAN_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/personal-loans"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/personal-loans" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/personal-loans" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = PERSONAL_LOAN_CATEGORIES[cat];
                  const count = getPersonalLoanCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/personal-loans/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/personal-loans" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="personal-loans" />
      </div>
    </div>
  );
}
