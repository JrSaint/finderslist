...
import type { PersonalLoanCategory } from "@/lib/personal-loans";
...
export async function generateStaticParams() {
  return getAllPersonalLoanCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = PERSONAL_LOAN_CATEGORIES[slug as PersonalLoanCategory];
  if (!category) return {};
  const tools = getPersonalLoanToolsByCategory(slug as PersonalLoanCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "personal loans", category.label],
    alternates: { canonical: `https://www.finderslist.com/personal-loans/category/${slug}` },
  };
}

export default async function PersonalLoanCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!PERSONAL_LOAN_CATEGORIES[slug as PersonalLoanCategory]) notFound();

  const category = PERSONAL_LOAN_CATEGORIES[slug as PersonalLoanCategory];
  const tools = getPersonalLoanToolsByCategory(slug as PersonalLoanCategory);
  const allCategories = getAllPersonalLoanCategories();
  const totalTools = getAllPersonalLoanTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/personal-loans/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/personal-loans" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
