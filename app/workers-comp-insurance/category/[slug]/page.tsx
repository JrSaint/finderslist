...
import type { WorkersCompCategory } from "@/lib/workers-comp-insurance";
...
export async function generateStaticParams() {
  return getAllWorkersCompCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = WORKERS_COMP_CATEGORIES[slug as WorkersCompCategory];
  if (!category) return {};
...
export default async function WorkersCompCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!WORKERS_COMP_CATEGORIES[slug as WorkersCompCategory]) notFound();

  const category = WORKERS_COMP_CATEGORIES[slug as WorkersCompCategory];
  const tools = getWorkersCompToolsByCategory(slug as WorkersCompCategory);
  const allCategories = getAllWorkersCompCategories();
  const totalTools = getAllWorkersCompTools().length;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `Best ${category.label} Workers Comp Insurance`,
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-6 text-sm text-slate-500">Showing {tools.length} provider{tools.length !== 1 ? "s" : ""}</div>
          <CategoryGuide guide={(category as { guide?: string }).guide} />
          {tools.length === 0 ? (
            <div className="text-center py-16 text-slate-500">No providers in this category yet.</div>
          ) : (
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/workers-comp-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              <p className="text-sm text-slate-400 mb-3">Know a great provider?</p>
              <Link href="/workers-comp-insurance" className="block w-full rounded-lg bg-orange-600 hover:bg-orange-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
              <p className="text-xs text-slate-600 mt-2">{totalTools} providers listed</p>
            </div>
          </div>
        </aside>
      </div>
