...
import type { InvestmentPlatformCategory } from "@/lib/investment-platforms";
...
export async function generateStaticParams() {
  return getAllInvestmentPlatformCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = INVESTMENT_PLATFORM_CATEGORIES[slug as InvestmentPlatformCategory];
  if (!category) return {};
  const tools = getInvestmentPlatformToolsByCategory(slug as InvestmentPlatformCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "investment platforms", category.label],
    alternates: { canonical: `https://www.finderslist.com/investment-platforms/category/${slug}` },
  };
}

export default async function InvestmentPlatformCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!INVESTMENT_PLATFORM_CATEGORIES[slug as InvestmentPlatformCategory]) notFound();

  const category = INVESTMENT_PLATFORM_CATEGORIES[slug as InvestmentPlatformCategory];
  const tools = getInvestmentPlatformToolsByCategory(slug as InvestmentPlatformCategory);
  const allCategories = getAllInvestmentPlatformCategories();
  const totalTools = getAllInvestmentPlatformTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/investment-platforms/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/investment-platforms/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/investment-platforms" className="block w-full rounded-lg bg-cyan-600 hover:bg-cyan-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
