...
import {
  getFeaturedOnlineTutoringTools,
  getAllOnlineTutoringTools,
  getAllOnlineTutoringCategories,
  ONLINE_TUTORING_CATEGORIES,
  getOnlineTutoringCategoryCount,
  filterOnlineTutoringTools,
} from "@/lib/online-tutoring-platforms";
import { ONLINE_TUTORING_EDITORIAL } from "@/data/online-tutoring-platforms";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/online-tutoring-platforms" } };
  }
  return {
    title: "Best Online Tutoring Platforms (2026) — 15+ Services Compared",
    description: "Compare the best online tutoring platforms for K-12, college, and test prep. Reviews of Wyzant, Varsity Tutors, Chegg, Khan Academy, and more.",
    keywords: ["online tutoring platforms","best online tutors 2026","virtual tutoring","homework help","math tutoring"],
    alternates: { canonical: "https://www.finderslist.com/online-tutoring-platforms" },
  };
}

export default async function OnlineTutoringPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedOnlineTutoringTools();
  const allTools = getAllOnlineTutoringTools();
  const categories = getAllOnlineTutoringCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterOnlineTutoringTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/online-tutoring-platforms/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ONLINE_TUTORING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 text-sm text-green-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated online tutoring platforms
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of online tutoring platforms. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/online-tutoring-platforms" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/online-tutoring-platforms"
        ...
        {!isFiltering && (
          <EditorialSection
            title={ONLINE_TUTORING_EDITORIAL.title}
            intro={ONLINE_TUTORING_EDITORIAL.intro}
            buyerGuide={ONLINE_TUTORING_EDITORIAL.buyerGuide}
            faq={ONLINE_TUTORING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/online-tutoring-platforms" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ONLINE_TUTORING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/online-tutoring-platforms"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/online-tutoring-platforms" className="text-green-400 hover:text-green-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/online-tutoring-platforms" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ONLINE_TUTORING_CATEGORIES[cat];
                  const count = getOnlineTutoringCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/online-tutoring-platforms/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/online-tutoring-platforms" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="online-tutoring-platforms" />
      </div>
    </div>
  );
}
