...
import type { CloudStorageCategory } from "@/lib/cloud-storage-providers";
...
export async function generateStaticParams() {
  return getAllCloudStorageCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = CLOUD_STORAGE_CATEGORIES[slug as CloudStorageCategory];
  if (!category) return {};
  const tools = getCloudStorageToolsByCategory(slug as CloudStorageCategory);
  return {
...
export default async function CloudStorageCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!CLOUD_STORAGE_CATEGORIES[slug as CloudStorageCategory]) notFound();

  const category = CLOUD_STORAGE_CATEGORIES[slug as CloudStorageCategory];
  const tools = getCloudStorageToolsByCategory(slug as CloudStorageCategory);
  const allCategories = getAllCloudStorageCategories();
  const totalTools = getAllCloudStorageTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/cloud-storage-providers/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/cloud-storage-providers" className="block w-full rounded-lg bg-sky-600 hover:bg-sky-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
