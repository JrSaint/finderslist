...
import {
  getFeaturedCloudStorageTools,
  getAllCloudStorageTools,
  getAllCloudStorageCategories,
  CLOUD_STORAGE_CATEGORIES,
  getCloudStorageCategoryCount,
  filterCloudStorageTools,
} from "@/lib/cloud-storage-providers";
import { CLOUD_STORAGE_EDITORIAL } from "@/data/cloud-storage-providers";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/cloud-storage-providers" } };
  }
  return {
    title: "Best Cloud Storage Providers (2026) — Personal, Business & Enterprise",
    description: "Compare the best cloud storage providers in 2026 for personal, business, and enterprise use. Reviews of Google Drive, Dropbox, OneDrive, Backblaze, and more.",
    keywords: ["cloud storage providers","best cloud storage 2026","online file storage","cloud backup","business cloud storage"],
    alternates: { canonical: "https://www.finderslist.com/cloud-storage-providers" },
  };
}

export default async function CloudStoragePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCloudStorageTools();
  const allTools = getAllCloudStorageTools();
  const categories = getAllCloudStorageCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCloudStorageTools({ query: q, pricing, role })
    : allTools;
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (CLOUD_STORAGE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-4 py-1.5 text-sm text-sky-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated cloud storage providers
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-5">
            <span className="text-white">Find the Right </span>
            <span className="bg-gradient-to-r from-sky-400 via-sky-300 to-sky-200 bg-clip-text text-transparent">
              Cloud Storage Providers
            </span>
          </h1>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of cloud storage providers. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/cloud-storage-providers" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/cloud-storage-providers"
        ...
        {!isFiltering && (
          <EditorialSection
            title={CLOUD_STORAGE_EDITORIAL.title}
            intro={CLOUD_STORAGE_EDITORIAL.intro}
            buyerGuide={CLOUD_STORAGE_EDITORIAL.buyerGuide}
            faq={CLOUD_STORAGE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/cloud-storage-providers" />
          <ComparisonTable
            tools={featured as never[]}
            categories={CLOUD_STORAGE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/cloud-storage-providers"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/cloud-storage-providers" className="text-sky-400 hover:text-sky-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/cloud-storage-providers" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = CLOUD_STORAGE_CATEGORIES[cat];
                  const count = getCloudStorageCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/cloud-storage-providers/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/cloud-storage-providers" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="cloud-storage-providers" />
      </div>
    </div>
  );
}
