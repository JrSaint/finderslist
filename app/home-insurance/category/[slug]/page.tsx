...
import type { HomeInsuranceCategory } from "@/lib/home-insurance";
...
export async function generateStaticParams() {
  return getAllHomeInsuranceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = HOME_INSURANCE_CATEGORIES[slug as HomeInsuranceCategory];
  if (!category) return {};
  const tools = getHomeInsuranceToolsByCategory(slug as HomeInsuranceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "home insurance", category.label],
    alternates: { canonical: `https://www.finderslist.com/home-insurance/category/${slug}` },
  };
}

export default async function HomeInsuranceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!HOME_INSURANCE_CATEGORIES[slug as HomeInsuranceCategory]) notFound();

  const category = HOME_INSURANCE_CATEGORIES[slug as HomeInsuranceCategory];
  const tools = getHomeInsuranceToolsByCategory(slug as HomeInsuranceCategory);
  const allCategories = getAllHomeInsuranceCategories();
  const totalTools = getAllHomeInsuranceTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/home-insurance/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/home-insurance/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/home-insurance" className="block w-full rounded-lg bg-sky-600 hover:bg-sky-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
