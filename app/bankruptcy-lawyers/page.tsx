...
import {
  getFeaturedBankruptcyLawyerTools,
  getAllBankruptcyLawyerTools,
  getAllBankruptcyLawyerCategories,
  BANKRUPTCY_LAWYER_CATEGORIES,
  getBankruptcyLawyerCategoryCount,
  filterBankruptcyLawyerTools,
} from "@/lib/bankruptcy-lawyers";
import { BANKRUPTCY_LAWYER_EDITORIAL } from "@/data/bankruptcy-lawyers";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/bankruptcy-lawyers" } };
  }
  return {
    title: "Best Bankruptcy Lawyers (2026) — 15+ Firms & Resources Compared",
    description: "Compare the best bankruptcy lawyers and debt relief firms. Find top-rated attorneys for Chapter 7, Chapter 13, Chapter 11, debt negotiation, and asset protection.",
    keywords: ["bankruptcy lawyer", "bankruptcy attorney", "Chapter 7 lawyer", "Chapter 13 attorney", "debt relief attorney", "bankruptcy law firm", "best bankruptcy lawyers 2026"],
    alternates: { canonical: "https://www.finderslist.com/bankruptcy-lawyers" },
  };
}

export default async function BankruptcyLawyersPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedBankruptcyLawyerTools();
  const allTools = getAllBankruptcyLawyerTools();
  const categories = getAllBankruptcyLawyerCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterBankruptcyLawyerTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Bankruptcy Lawyers", item: "https://www.finderslist.com/bankruptcy-lawyers" },
    ],
  };
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (BANKRUPTCY_LAWYER_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-4 py-1.5 text-sm text-blue-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated bankruptcy law firms & resources
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={BANKRUPTCY_LAWYER_EDITORIAL.title}
            intro={BANKRUPTCY_LAWYER_EDITORIAL.intro}
            buyerGuide={BANKRUPTCY_LAWYER_EDITORIAL.buyerGuide}
            faq={BANKRUPTCY_LAWYER_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/bankruptcy-lawyers" />
          <ComparisonTable
            tools={featured as never[]}
            categories={BANKRUPTCY_LAWYER_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/bankruptcy-lawyers"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/bankruptcy-lawyers" className="text-blue-400 hover:text-blue-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getBankruptcyLawyerCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/bankruptcy-lawyers" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="bankruptcy-lawyers" />
      </div>
    </div>
  );
}
