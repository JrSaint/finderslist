...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/bankruptcy-lawyers/tools/${tool.slug}` },
  };
}
...
export default async function BankruptcyLawyerToolPage({ params }: Props) {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/bankruptcy-lawyers/category/${tool.category}` },
        ...
        <div className="lg:w-80 flex-shrink-0 space-y-6">
          <div className="rounded-xl border border-white/10 bg-slate-900/60 p-5">
            <a
              ...
              aria-disabled={isDead || undefined} className={`block w-full text-center py-3 px-4 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium text-sm transition-colors mb-4${isDead ? " opacity-50 grayscale pointer-events-none" : ""}`}
            >
