...
import {
  getFeaturedFinanceTools,
  getAllFinanceTools,
  getAllFinanceCategories,
  FINANCE_CATEGORIES,
  getFinanceCategoryCount,
  filterFinanceTools,
  FINANCE_ROLES,
} from "@/lib/finance-tools";
import { FINANCE_EDITORIAL } from "@/data/finance-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/finance-tools" } };
  }
  return {
    title: "Best Finance & Accounting Tools Directory (2026) — 35+ Tools Reviewed",
    description:
      "Discover the best finance and accounting tools for small businesses, freelancers, and startups. Compare QuickBooks, Xero, Wave, FreshBooks, Gusto, and 30+ more with honest reviews, pricing, and real-world use cases.",
    keywords: [
      "accounting software",
      "best accounting tools 2026",
      "invoicing software",
      "payroll software",
      "expense tracking",
      "business banking",
      "finance tools directory",
    ],
    alternates: { canonical: "https://www.finderslist.com/finance-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(FINANCE_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function FinanceToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedFinanceTools();
  const allTools = getAllFinanceTools();
  const categories = getAllFinanceCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterFinanceTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Finance & Accounting Tools", item: "https://www.finderslist.com/finance-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    description: "Discover the best finance and accounting tools for small businesses, freelancers, and startups. Compare QuickBooks, Xero, Wave, FreshBooks, Gusto, and 30+ more with honest reviews, pricing, and real-world use cases.",
    numberOfItems: allTools.length,
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      "@type": "ListItem",
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/finance-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (FINANCE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated finance & accounting tools
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={FINANCE_EDITORIAL.title}
            intro={FINANCE_EDITORIAL.intro}
            buyerGuide={FINANCE_EDITORIAL.buyerGuide}
            faq={FINANCE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/finance-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={FINANCE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/finance-tools"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/finance-tools" className="text-amber-400 hover:text-amber-300 text-sm font-medium">
                  Clear all filters →
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/finance-tools" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = FINANCE_CATEGORIES[cat];
                  const count = getFinanceCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/finance-tools/category/${cat}`}
                    ...
                    >
                      ...
                      <span className="text-sm font-medium text-white group-hover:text-amber-300 transition-colors leading-tight">
                        {info.label}
                      </span>
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/finance-tools" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="finance-tools" />
      </div>
    </div>
  );
}
