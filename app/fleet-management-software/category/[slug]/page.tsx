...
import type { FleetManagementCategory } from "@/lib/fleet-management-software";
...
export async function generateStaticParams() {
  return getAllFleetManagementCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = FLEET_MANAGEMENT_CATEGORIES[slug as FleetManagementCategory];
  if (!category) return {};
  const tools = getFleetManagementToolsByCategory(slug as FleetManagementCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "fleet management software", category.label],
    alternates: { canonical: `https://www.finderslist.com/fleet-management-software/category/${slug}` },
  };
}

export default async function FleetManagementCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!FLEET_MANAGEMENT_CATEGORIES[slug as FleetManagementCategory]) notFound();

  const category = FLEET_MANAGEMENT_CATEGORIES[slug as FleetManagementCategory];
  const tools = getFleetManagementToolsByCategory(slug as FleetManagementCategory);
  const allCategories = getAllFleetManagementCategories();
  const totalTools = getAllFleetManagementTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/fleet-management-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/fleet-management-software" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
