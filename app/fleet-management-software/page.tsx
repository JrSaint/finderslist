...
import {
  getFeaturedFleetManagementTools,
  getAllFleetManagementTools,
  getAllFleetManagementCategories,
  FLEET_MANAGEMENT_CATEGORIES,
  getFleetManagementCategoryCount,
  filterFleetManagementTools,
} from "@/lib/fleet-management-software";
import { FLEET_MANAGEMENT_EDITORIAL } from "@/data/fleet-management-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/fleet-management-software" } };
  }
  return {
    title: "Best Fleet Management Software (2026) — 15+ Platforms Compared",
    description: "Compare the best fleet management and GPS tracking software. Reviews of Samsara, Verizon Connect, Geotab, Motive, and more.",
    keywords: ["fleet management software","gps fleet tracking 2026","vehicle tracking","fleet maintenance","telematics"],
    alternates: { canonical: "https://www.finderslist.com/fleet-management-software" },
  };
}

export default async function FleetManagementPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedFleetManagementTools();
  const allTools = getAllFleetManagementTools();
  const categories = getAllFleetManagementCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterFleetManagementTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/fleet-management-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (FLEET_MANAGEMENT_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-4 py-1.5 text-sm text-emerald-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated fleet management software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of fleet management software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/fleet-management-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/fleet-management-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={FLEET_MANAGEMENT_EDITORIAL.title}
            intro={FLEET_MANAGEMENT_EDITORIAL.intro}
            buyerGuide={FLEET_MANAGEMENT_EDITORIAL.buyerGuide}
            faq={FLEET_MANAGEMENT_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/fleet-management-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={FLEET_MANAGEMENT_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/fleet-management-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/fleet-management-software" className="text-emerald-400 hover:text-emerald-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/fleet-management-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = FLEET_MANAGEMENT_CATEGORIES[cat];
                  const count = getFleetManagementCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/fleet-management-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/fleet-management-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="fleet-management-software" />
      </div>
    </div>
  );
}
