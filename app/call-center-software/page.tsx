...
import {
  getFeaturedCallCenterTools,
  getAllCallCenterTools,
  getAllCallCenterCategories,
  CALL_CENTER_CATEGORIES,
  getCallCenterCategoryCount,
  filterCallCenterTools,
} from "@/lib/call-center-software";
import { CALL_CENTER_EDITORIAL } from "@/data/call-center-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/call-center-software" } };
  }
  return {
    title: "Best Call Center Software (2026) — Cloud Contact Center Platforms Compared",
    description: "Compare the best call center and contact center software in 2026. Reviews of cloud contact centers, predictive dialers, IVR, and workforce management platforms.",
    keywords: ["call center software","contact center platform","best call center software 2026","cloud contact center","predictive dialer"],
    alternates: { canonical: "https://www.finderslist.com/call-center-software" },
  };
}

export default async function CallCenterPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCallCenterTools();
  const allTools = getAllCallCenterTools();
  const categories = getAllCallCenterCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCallCenterTools({ query: q, pricing, role })
    : allTools;
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (CALL_CENTER_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-4 py-1.5 text-sm text-blue-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated call center software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of call center software. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/call-center-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={CALL_CENTER_EDITORIAL.title}
            intro={CALL_CENTER_EDITORIAL.intro}
            buyerGuide={CALL_CENTER_EDITORIAL.buyerGuide}
            faq={CALL_CENTER_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/call-center-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={CALL_CENTER_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/call-center-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/call-center-software" className="text-blue-400 hover:text-blue-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/call-center-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = CALL_CENTER_CATEGORIES[cat];
                  const count = getCallCenterCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/call-center-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/call-center-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="call-center-software" />
      </div>
    </div>
  );
}
