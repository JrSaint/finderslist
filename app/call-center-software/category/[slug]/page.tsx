...
import type { CallCenterCategory } from "@/lib/call-center-software";
...
export async function generateStaticParams() {
  return getAllCallCenterCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = CALL_CENTER_CATEGORIES[slug as CallCenterCategory];
  if (!category) return {};
  const tools = getCallCenterToolsByCategory(slug as CallCenterCategory);
  return {
...
export default async function CallCenterCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!CALL_CENTER_CATEGORIES[slug as CallCenterCategory]) notFound();

  const category = CALL_CENTER_CATEGORIES[slug as CallCenterCategory];
  const tools = getCallCenterToolsByCategory(slug as CallCenterCategory);
  const allCategories = getAllCallCenterCategories();
  const totalTools = getAllCallCenterTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/call-center-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/call-center-software" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
