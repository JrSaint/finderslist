...
import type { MarketingAutomationCategory } from "@/lib/marketing-automation-software";
...
export async function generateStaticParams() {
  return getAllMarketingAutomationCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = MARKETING_AUTOMATION_CATEGORIES[slug as MarketingAutomationCategory];
  if (!category) return {};
  const tools = getMarketingAutomationToolsByCategory(slug as MarketingAutomationCategory);
  return {
    ...
    keywords: [`${category.label}`, `best ${category.label} 2026`, "marketing automation software", category.label],
    alternates: { canonical: `https://finderslist.com/marketing-automation-software/category/${slug}` },
  };
}

export default async function MarketingAutomationCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!MARKETING_AUTOMATION_CATEGORIES[slug as MarketingAutomationCategory]) notFound();

  const category = MARKETING_AUTOMATION_CATEGORIES[slug as MarketingAutomationCategory];
  const tools = getMarketingAutomationToolsByCategory(slug as MarketingAutomationCategory);
  const allCategories = getAllMarketingAutomationCategories();
  const totalTools = getAllMarketingAutomationTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://finderslist.com/marketing-automation-software/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/marketing-automation-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/marketing-automation-software" className="block w-full rounded-lg bg-fuchsia-600 hover:bg-fuchsia-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
