...
import {
  getFeaturedAnalyticsTools,
  getAllAnalyticsTools,
  getAllAnalyticsCategories,
  ANALYTICS_CATEGORIES,
  getAnalyticsCategoryCount,
  filterAnalyticsTools,
  ANALYTICS_ROLES,
} from "@/lib/analytics-tools";
import { ANALYTICS_EDITORIAL } from "@/data/analytics-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/analytics-tools" } };
  }
  return {
    title: "Best Analytics & BI Tools Directory (2026) — 30+ Tools Reviewed",
    description: "The most comprehensive directory of analytics and business intelligence tools. Compare Mixpanel, Amplitude, Tableau, Hotjar, Plausible, Looker, and 30+ more.",
    alternates: { canonical: "https://www.finderslist.com/analytics-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(ANALYTICS_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function AnalyticsToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedAnalyticsTools();
  const allTools = getAllAnalyticsTools();
  const categories = getAllAnalyticsCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterAnalyticsTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Analytics & BI Tools", item: "https://www.finderslist.com/analytics-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/analytics-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ANALYTICS_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={ANALYTICS_EDITORIAL.title}
            intro={ANALYTICS_EDITORIAL.intro}
            buyerGuide={ANALYTICS_EDITORIAL.buyerGuide}
            faq={ANALYTICS_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/analytics-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ANALYTICS_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/analytics-tools"
          />
          </>
        )}
        ...
        {isFiltering ? (
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/analytics-tools" className="text-cyan-400 hover:text-cyan-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/analytics-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getAnalyticsCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/analytics-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-cyan-500/30 transition-all group">
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {featured.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/analytics-tools" />))}
              </div>
            </section>
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/analytics-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="analytics-tools" />
      </div>
    </div>
  );
}
