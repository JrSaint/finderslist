...
import {
  getFeaturedTravelInsuranceTools,
  getAllTravelInsuranceTools,
  getAllTravelInsuranceCategories,
  TRAVEL_INSURANCE_CATEGORIES,
  getTravelInsuranceCategoryCount,
  filterTravelInsuranceTools,
} from "@/lib/travel-insurance";
import { TRAVEL_INSURANCE_EDITORIAL } from "@/data/travel-insurance";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/travel-insurance" } };
  }
  return {
    title: "Best Travel Insurance Companies (2026) — 15+ Providers Compared",
    description: "Compare the best travel insurance companies for trip cancellation, medical coverage, and emergency evacuation. Reviews of Allianz, World Nomads, and more.",
    keywords: ["travel insurance","best travel insurance 2026","trip cancellation insurance","travel medical insurance","travel protection"],
    alternates: { canonical: "https://www.finderslist.com/travel-insurance" },
  };
}

export default async function TravelInsurancePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTravelInsuranceTools();
  const allTools = getAllTravelInsuranceTools();
  const categories = getAllTravelInsuranceCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterTravelInsuranceTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Travel Insurance Companies", item: "https://www.finderslist.com/travel-insurance" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/travel-insurance/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TRAVEL_INSURANCE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-4 py-1.5 text-sm text-sky-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated travel insurance
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of travel insurance. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/travel-insurance" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={TRAVEL_INSURANCE_EDITORIAL.title}
            intro={TRAVEL_INSURANCE_EDITORIAL.intro}
            buyerGuide={TRAVEL_INSURANCE_EDITORIAL.buyerGuide}
            faq={TRAVEL_INSURANCE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/travel-insurance" />
          <ComparisonTable
            tools={featured as never[]}
            categories={TRAVEL_INSURANCE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/travel-insurance"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/travel-insurance" className="text-sky-400 hover:text-sky-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/travel-insurance" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = TRAVEL_INSURANCE_CATEGORIES[cat];
                  const count = getTravelInsuranceCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/travel-insurance/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/travel-insurance" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="travel-insurance" />
      </div>
    </div>
  );
}
