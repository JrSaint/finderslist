...
import {
  getFeaturedSocialMediaTools,
  getAllSocialMediaTools,
  getAllSocialMediaCategories,
  SOCIAL_MEDIA_CATEGORIES,
  getSocialMediaCategoryCount,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/social-media-tools" } };
  }
  return {
    title: "Best Social Media Management Tools Directory (2026) — 30+ Tools Reviewed",
    description: "The most comprehensive directory of social media management tools. Compare Hootsuite, Buffer, Sprout Social, Later, and 30+ more with honest reviews.",
    alternates: { canonical: "https://www.finderslist.com/social-media-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(SOCIAL_MEDIA_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function SocialMediaToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSocialMediaTools();
  const allTools = getAllSocialMediaTools();
  const categories = getAllSocialMediaCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSocialMediaTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Social Media Management Tools", item: "https://www.finderslist.com/social-media-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/social-media-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (SOCIAL_MEDIA_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/social-media-tools" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/social-media-tools" className="text-pink-400 hover:text-pink-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/social-media-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = SOCIAL_MEDIA_CATEGORIES[cat];
                  const count = getSocialMediaCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/social-media-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-pink-500/30 transition-all group">
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/social-media-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="social-media-tools" />
      </div>
    </div>
  );
}
