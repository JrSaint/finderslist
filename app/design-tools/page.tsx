...
import {
  getFeaturedDesignTools,
  getAllDesignTools,
  getAllDesignCategories,
  DESIGN_CATEGORIES,
  getDesignCategoryCount,
  filterDesignTools,
  DESIGN_ROLES,
} from "@/lib/design-tools";
import { DESIGN_EDITORIAL } from "@/data/design-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/design-tools" } };
  }
  return {
    title: "Best Design Tools Directory (2026) — 30+ Tools Reviewed",
    description: "The most comprehensive directory of design and creative tools. Compare Figma, Canva, Adobe CC, Sketch, Framer, Miro, and 30+ more with honest reviews.",
    alternates: { canonical: "https://www.finderslist.com/design-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(DESIGN_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function DesignToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedDesignTools();
  const allTools = getAllDesignTools();
  const categories = getAllDesignCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterDesignTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/design-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (DESIGN_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight mb-5">
            ...
            <span className="bg-gradient-to-r from-fuchsia-400 via-fuchsia-300 to-fuchsia-500 bg-clip-text text-transparent">
              Design Tools
            </span>
          </h1>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={DESIGN_EDITORIAL.title}
            intro={DESIGN_EDITORIAL.intro}
            buyerGuide={DESIGN_EDITORIAL.buyerGuide}
            faq={DESIGN_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/design-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DESIGN_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/design-tools"
          />
          </>
        )}
        ...
        {isFiltering ? (
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/design-tools" className="text-fuchsia-400 hover:text-fuchsia-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/design-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getDesignCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/design-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-fuchsia-500/30 transition-all group">
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {featured.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/design-tools" />))}
              </div>
            </section>
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/design-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="design-tools" />
      </div>
    </div>
  );
}
