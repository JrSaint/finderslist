...
import { ATS_EDITORIAL } from "@/data/applicant-tracking-systems";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/applicant-tracking-systems" } };
  }
  return {
    title: "Best Applicant Tracking Systems (2026) — 20+ ATS Reviewed",
    description: "Compare the best applicant tracking systems for hiring teams. Reviews of Greenhouse, Lever, Workable, JazzHR, and 15+ more ATS platforms.",
    keywords: ["applicant tracking system","best ats 2026","recruiting software","hiring software","talent acquisition platform"],
    alternates: { canonical: "https://www.finderslist.com/applicant-tracking-systems" },
  };
}

export default async function ATSPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedATSTools();
  const allTools = getAllATSTools();
  const categories = getAllATSCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterATSTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/applicant-tracking-systems/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ATS_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-sky-500/30 bg-sky-500/10 px-4 py-1.5 text-sm text-sky-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated applicant tracking systems
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of applicant tracking systems. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/applicant-tracking-systems" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/applicant-tracking-systems"
        ...
        {!isFiltering && (
          <EditorialSection
            title={ATS_EDITORIAL.title}
            intro={ATS_EDITORIAL.intro}
            buyerGuide={ATS_EDITORIAL.buyerGuide}
            faq={ATS_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/applicant-tracking-systems" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ATS_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/applicant-tracking-systems"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/applicant-tracking-systems" className="text-sky-400 hover:text-sky-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/applicant-tracking-systems" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ATS_CATEGORIES[cat];
                  const count = getATSCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/applicant-tracking-systems/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/applicant-tracking-systems" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="applicant-tracking-systems" />
      </div>
    </div>
  );
}
