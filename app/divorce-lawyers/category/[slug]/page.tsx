...
import type { DivorceLawyerCategory } from "@/lib/divorce-lawyers";
...
export async function generateStaticParams() {
  return getAllDivorceLawyerCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = DIVORCE_LAWYER_CATEGORIES[slug as DivorceLawyerCategory];
  if (!category) return {};
  const tools = getDivorceLawyerToolsByCategory(slug as DivorceLawyerCategory);
  return {
    ...
    keywords: [`${category.label}`, `best ${category.label} 2026`, "divorce lawyers", category.label],
    alternates: { canonical: `https://www.finderslist.com/divorce-lawyers/category/${slug}` },
  };
}

export default async function DivorceLawyerCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!DIVORCE_LAWYER_CATEGORIES[slug as DivorceLawyerCategory]) notFound();

  const category = DIVORCE_LAWYER_CATEGORIES[slug as DivorceLawyerCategory];
  const tools = getDivorceLawyerToolsByCategory(slug as DivorceLawyerCategory);
  const allCategories = getAllDivorceLawyerCategories();
  const totalTools = getAllDivorceLawyerTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/divorce-lawyers/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/divorce-lawyers" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
