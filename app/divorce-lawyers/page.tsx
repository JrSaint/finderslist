...
import {
  getFeaturedDivorceLawyerTools,
  getAllDivorceLawyerTools,
  getAllDivorceLawyerCategories,
  DIVORCE_LAWYER_CATEGORIES,
  getDivorceLawyerCategoryCount,
  filterDivorceLawyerTools,
} from "@/lib/divorce-lawyers";
import { DIVORCE_LAWYER_EDITORIAL } from "@/data/divorce-lawyers";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/divorce-lawyers" } };
  }
  return {
    title: "Best Divorce Lawyers (2026) — Family Law Attorneys Compared",
    description: "Find top-rated divorce lawyers in 2026 for contested, uncontested, child custody, asset division, and mediation cases. Compare family law firms and attorneys.",
    keywords: ["divorce lawyers","family law attorneys","best divorce lawyer 2026","child custody attorney","divorce mediation"],
    alternates: { canonical: "https://www.finderslist.com/divorce-lawyers" },
  };
}

export default async function DivorceLawyerPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedDivorceLawyerTools();
  const allTools = getAllDivorceLawyerTools();
  const categories = getAllDivorceLawyerCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterDivorceLawyerTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/divorce-lawyers/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (DIVORCE_LAWYER_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-sm text-violet-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated divorce lawyers
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of divorce lawyers. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/divorce-lawyers" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={DIVORCE_LAWYER_EDITORIAL.title}
            intro={DIVORCE_LAWYER_EDITORIAL.intro}
            buyerGuide={DIVORCE_LAWYER_EDITORIAL.buyerGuide}
            faq={DIVORCE_LAWYER_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/divorce-lawyers" />
          <ComparisonTable
            tools={featured as never[]}
            categories={DIVORCE_LAWYER_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/divorce-lawyers"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/divorce-lawyers" className="text-violet-400 hover:text-violet-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/divorce-lawyers" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = DIVORCE_LAWYER_CATEGORIES[cat];
                  const count = getDivorceLawyerCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/divorce-lawyers/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/divorce-lawyers" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="divorce-lawyers" />
      </div>
    </div>
  );
}
