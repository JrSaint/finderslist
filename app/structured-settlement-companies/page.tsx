...
import {
  getFeaturedStructuredSettlementTools,
...
export default async function StructuredSettlementCompaniesPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedStructuredSettlementTools();
  const allTools = getAllStructuredSettlementTools();
  const categories = getAllStructuredSettlementCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterStructuredSettlementTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Structured Settlement Companies",
    description: "Compare the best structured settlement companies for selling your payments. Reviews of JG Wentworth, Peachtree Financial, Fairfield Funding, and more.",
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-fuchsia-500/30 bg-fuchsia-500/10 px-4 py-1.5 text-sm text-fuchsia-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated structured settlement companies
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <section className="mt-10 mb-16">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-semibold text-white">
                  {q ? `Results for "${q}"` : `${pricing?.charAt(0).toUpperCase()}${pricing?.slice(1)} Companies`}
                </h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  {filteredTools.length} compan{filteredTools.length !== 1 ? "ies" : "y"} found
                </p>
              </div>
            </div>
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <p className="text-lg font-medium text-slate-300 mb-2">No companies found</p>
                <p className="text-sm mb-6">Try a different search term or clear your filters</p>
                <Link href="/structured-settlement-companies" className="text-fuchsia-400 hover:text-fuchsia-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getStructuredSettlementCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/structured-settlement-companies/category/${cat}`}
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-600">{count} companies</span>
                    </Link>
                  );
                })}
              </div>
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/structured-settlement-companies" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="structured-settlement-companies" />
      </div>
    </div>
  );
}
