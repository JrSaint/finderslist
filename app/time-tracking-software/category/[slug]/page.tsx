...
import type { TimeTrackingCategory } from "@/lib/time-tracking-software";
...
export async function generateStaticParams() {
  return getAllTimeTrackingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = TIME_TRACKING_CATEGORIES[slug as TimeTrackingCategory];
  if (!category) return {};
  const tools = getTimeTrackingToolsByCategory(slug as TimeTrackingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "time tracking software", category.label],
    alternates: { canonical: `https://www.finderslist.com/time-tracking-software/category/${slug}` },
  };
}

export default async function TimeTrackingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!TIME_TRACKING_CATEGORIES[slug as TimeTrackingCategory]) notFound();

  const category = TIME_TRACKING_CATEGORIES[slug as TimeTrackingCategory];
  const tools = getTimeTrackingToolsByCategory(slug as TimeTrackingCategory);
  const allCategories = getAllTimeTrackingCategories();
  const totalTools = getAllTimeTrackingTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/time-tracking-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/time-tracking-software" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
