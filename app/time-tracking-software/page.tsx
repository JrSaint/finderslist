...
import {
  getFeaturedTimeTrackingTools,
  getAllTimeTrackingTools,
  getAllTimeTrackingCategories,
  TIME_TRACKING_CATEGORIES,
  getTimeTrackingCategoryCount,
  filterTimeTrackingTools,
} from "@/lib/time-tracking-software";
import { TIME_TRACKING_EDITORIAL } from "@/data/time-tracking-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/time-tracking-software" } };
  }
  return {
    title: "Best Time Tracking Software (2026) — 20+ Tools Reviewed",
    description: "Compare the best time tracking software for freelancers, agencies, and remote teams. Reviews of Toggl, Harvest, Clockify, Time Doctor, and more.",
    keywords: ["time tracking software","best time tracker 2026","employee time tracking","project time management","billable hours tracking"],
    alternates: { canonical: "https://www.finderslist.com/time-tracking-software" },
  };
}

export default async function TimeTrackingPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTimeTrackingTools();
  const allTools = getAllTimeTrackingTools();
  const categories = getAllTimeTrackingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterTimeTrackingTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/time-tracking-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TIME_TRACKING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated time tracking software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of time tracking software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/time-tracking-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={TIME_TRACKING_EDITORIAL.title}
            intro={TIME_TRACKING_EDITORIAL.intro}
            buyerGuide={TIME_TRACKING_EDITORIAL.buyerGuide}
            faq={TIME_TRACKING_EDITORIAL.faq}
          />
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/time-tracking-software" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/time-tracking-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = TIME_TRACKING_CATEGORIES[cat];
                  const count = getTimeTrackingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/time-tracking-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/time-tracking-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="time-tracking-software" />
      </div>
    </div>
  );
}
