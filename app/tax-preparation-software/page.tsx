...
import {
  getFeaturedTaxPrepTools,
  getAllTaxPrepTools,
  getAllTaxPrepCategories,
  TAX_PREP_CATEGORIES,
  getTaxPrepCategoryCount,
  filterTaxPrepTools,
} from "@/lib/tax-preparation-software";
import { TAX_PREP_EDITORIAL } from "@/data/tax-preparation-software";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/tax-preparation-software" } };
  }
  return {
    title: "Best Tax Preparation Software (2026) — 15+ Options Compared",
    description: "Compare the best tax preparation software for individuals, freelancers, and small businesses. Reviews of TurboTax, H&R Block, TaxAct, and more.",
    keywords: ["tax preparation software","best tax software 2026","online tax filing","tax return software","free tax filing"],
    alternates: { canonical: "https://www.finderslist.com/tax-preparation-software" },
  };
}

export default async function TaxPrepPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTaxPrepTools();
  const allTools = getAllTaxPrepTools();
  const categories = getAllTaxPrepCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterTaxPrepTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/tax-preparation-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TAX_PREP_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-4 py-1.5 text-sm text-green-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated tax preparation software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of tax preparation software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/tax-preparation-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/tax-preparation-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={TAX_PREP_EDITORIAL.title}
            intro={TAX_PREP_EDITORIAL.intro}
            buyerGuide={TAX_PREP_EDITORIAL.buyerGuide}
            faq={TAX_PREP_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/tax-preparation-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={TAX_PREP_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/tax-preparation-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/tax-preparation-software" className="text-green-400 hover:text-green-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/tax-preparation-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = TAX_PREP_CATEGORIES[cat];
                  const count = getTaxPrepCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/tax-preparation-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/tax-preparation-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="tax-preparation-software" />
      </div>
    </div>
  );
}
