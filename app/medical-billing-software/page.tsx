...
import {
  getFeaturedMedicalBillingTools,
  getAllMedicalBillingTools,
  getAllMedicalBillingCategories,
  MEDICAL_BILLING_CATEGORIES,
  getMedicalBillingCategoryCount,
  filterMedicalBillingTools,
} from "@/lib/medical-billing-software";
import { MEDICAL_BILLING_EDITORIAL } from "@/data/medical-billing-software";
...
export default async function MedicalBillingSoftwarePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedMedicalBillingTools();
  const allTools = getAllMedicalBillingTools();
  const categories = getAllMedicalBillingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterMedicalBillingTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/medical-billing-software/tools/${tool.slug}`,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-rose-500/30 bg-rose-500/10 px-4 py-1.5 text-sm text-rose-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated medical billing software
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/medical-billing-software"
        ...
        {!isFiltering && (
          <EditorialSection
            title={MEDICAL_BILLING_EDITORIAL.title}
            intro={MEDICAL_BILLING_EDITORIAL.intro}
            buyerGuide={MEDICAL_BILLING_EDITORIAL.buyerGuide}
            faq={MEDICAL_BILLING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/medical-billing-software" />
          <ComparisonTable
            tools={featured as never[]}
            categories={MEDICAL_BILLING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/medical-billing-software"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/medical-billing-software" className="text-rose-400 hover:text-rose-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/medical-billing-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = MEDICAL_BILLING_CATEGORIES[cat];
                  const count = getMedicalBillingCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/medical-billing-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/medical-billing-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="medical-billing-software" />
      </div>
    </div>
  );
}
