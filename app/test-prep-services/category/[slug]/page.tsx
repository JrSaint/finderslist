...
import type { TestPrepCategory } from "@/lib/test-prep-services";
...
export async function generateStaticParams() {
  return getAllTestPrepCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = TEST_PREP_CATEGORIES[slug as TestPrepCategory];
  if (!category) return {};
  const tools = getTestPrepToolsByCategory(slug as TestPrepCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "test prep services", category.label],
    alternates: { canonical: `https://www.finderslist.com/test-prep-services/category/${slug}` },
  };
}

export default async function TestPrepCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!TEST_PREP_CATEGORIES[slug as TestPrepCategory]) notFound();

  const category = TEST_PREP_CATEGORIES[slug as TestPrepCategory];
  const tools = getTestPrepToolsByCategory(slug as TestPrepCategory);
  const allCategories = getAllTestPrepCategories();
  const totalTools = getAllTestPrepTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/test-prep-services/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/test-prep-services" className="block w-full rounded-lg bg-indigo-600 hover:bg-indigo-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
