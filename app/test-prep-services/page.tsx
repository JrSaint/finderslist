...
import {
  getFeaturedTestPrepTools,
  getAllTestPrepTools,
  getAllTestPrepCategories,
  TEST_PREP_CATEGORIES,
  getTestPrepCategoryCount,
  filterTestPrepTools,
} from "@/lib/test-prep-services";
import { TEST_PREP_EDITORIAL } from "@/data/test-prep-services";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/test-prep-services" } };
  }
  return {
    title: "Best Test Prep Services (2026) — 15+ SAT, GRE, LSAT Options Compared",
    description: "Compare the best test prep services for SAT, ACT, GRE, GMAT, and LSAT. Reviews of Kaplan, Princeton Review, Magoosh, Khan Academy, and more.",
    keywords: ["test prep services","best test prep 2026","sat prep","gre prep","lsat prep"],
    alternates: { canonical: "https://www.finderslist.com/test-prep-services" },
  };
}

export default async function TestPrepPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedTestPrepTools();
  const allTools = getAllTestPrepTools();
  const categories = getAllTestPrepCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterTestPrepTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/test-prep-services/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (TEST_PREP_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-500/30 bg-indigo-500/10 px-4 py-1.5 text-sm text-indigo-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated test prep services
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={TEST_PREP_EDITORIAL.title}
            intro={TEST_PREP_EDITORIAL.intro}
            buyerGuide={TEST_PREP_EDITORIAL.buyerGuide}
            faq={TEST_PREP_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/test-prep-services" />
          <ComparisonTable
            tools={featured as never[]}
            categories={TEST_PREP_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/test-prep-services"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/test-prep-services" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/test-prep-services" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = TEST_PREP_CATEGORIES[cat];
                  const count = getTestPrepCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/test-prep-services/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/test-prep-services" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="test-prep-services" />
      </div>
    </div>
  );
}
