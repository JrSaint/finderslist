...
import type { ProjectManagementCategory } from "@/lib/project-management-software";
...
export async function generateStaticParams() {
  return getAllProjectManagementCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = PROJECT_MANAGEMENT_CATEGORIES[slug as ProjectManagementCategory];
  if (!category) return {};
  const tools = getProjectManagementToolsByCategory(slug as ProjectManagementCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "project management software", category.label],
    alternates: { canonical: `https://www.finderslist.com/project-management-software/category/${slug}` },
  };
}

export default async function ProjectManagementCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!PROJECT_MANAGEMENT_CATEGORIES[slug as ProjectManagementCategory]) notFound();

  const category = PROJECT_MANAGEMENT_CATEGORIES[slug as ProjectManagementCategory];
  const tools = getProjectManagementToolsByCategory(slug as ProjectManagementCategory);
  const allCategories = getAllProjectManagementCategories();
  const totalTools = getAllProjectManagementTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/project-management-software/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/project-management-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/project-management-software" className="block w-full rounded-lg bg-violet-600 hover:bg-violet-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
