...
import {
  getFeaturedMentalHealthTools,
  getAllMentalHealthTools,
  getAllMentalHealthCategories,
  MENTAL_HEALTH_CATEGORIES,
  getMentalHealthCategoryCount,
  filterMentalHealthTools,
} from "@/lib/mental-health-apps";
import { MENTAL_HEALTH_EDITORIAL } from "@/data/mental-health-apps";
...
export default async function MentalHealthPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedMentalHealthTools();
  const allTools = getAllMentalHealthTools();
  const categories = getAllMentalHealthCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterMentalHealthTools({ query: q, pricing, role })
    : allTools;
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-purple-500/30 bg-purple-500/10 px-4 py-1.5 text-sm text-purple-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated mental health apps
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={MENTAL_HEALTH_EDITORIAL.title}
            intro={MENTAL_HEALTH_EDITORIAL.intro}
            buyerGuide={MENTAL_HEALTH_EDITORIAL.buyerGuide}
            faq={MENTAL_HEALTH_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/mental-health-apps" />
          <ComparisonTable
            tools={featured as never[]}
            categories={MENTAL_HEALTH_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/mental-health-apps"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/mental-health-apps" className="text-purple-400 hover:text-purple-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  ...
                  const count = getMentalHealthCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/mental-health-apps/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/mental-health-apps" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="mental-health-apps" />
      </div>
    </div>
  );
}
