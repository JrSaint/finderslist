...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/personal-injury-lawyers" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/personal-injury-lawyers" },
  };
}
...
export default async function PersonalInjuryLawyersPage({ searchParams }: Props) {
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/personal-injury-lawyers/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (PERSONAL_INJURY_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/personal-injury-lawyers" />
          <ComparisonTable
        ...
        <RelatedDirectories currentSlug="personal-injury-lawyers" />
      </div>
    </div>
  );
}
