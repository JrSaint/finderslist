...
import {
  getFeaturedCryptoExchangeTools,
  getAllCryptoExchangeTools,
  getAllCryptoExchangeCategories,
  CRYPTO_EXCHANGE_CATEGORIES,
  getCryptoExchangeCategoryCount,
  filterCryptoExchangeTools,
} from "@/lib/cryptocurrency-exchanges";
import { CRYPTO_EXCHANGE_EDITORIAL } from "@/data/cryptocurrency-exchanges";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/cryptocurrency-exchanges" } };
  }
  return {
    title: "Best Cryptocurrency Exchanges (2026) — 15+ Platforms Compared",
    description: "Compare the best cryptocurrency exchanges for trading Bitcoin, Ethereum, and altcoins. Reviews of Coinbase, Kraken, Binance, Gemini, and more.",
    keywords: ["cryptocurrency exchanges","best crypto exchanges 2026","bitcoin trading","crypto trading platforms","buy cryptocurrency"],
    alternates: { canonical: "https://www.finderslist.com/cryptocurrency-exchanges" },
  };
}

export default async function CryptoExchangePage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedCryptoExchangeTools();
  const allTools = getAllCryptoExchangeTools();
  const categories = getAllCryptoExchangeCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterCryptoExchangeTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/cryptocurrency-exchanges/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (CRYPTO_EXCHANGE_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-1.5 text-sm text-orange-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated cryptocurrency exchanges
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of cryptocurrency exchanges. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/cryptocurrency-exchanges" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/cryptocurrency-exchanges"
        ...
        {!isFiltering && (
          <EditorialSection
            title={CRYPTO_EXCHANGE_EDITORIAL.title}
            intro={CRYPTO_EXCHANGE_EDITORIAL.intro}
            buyerGuide={CRYPTO_EXCHANGE_EDITORIAL.buyerGuide}
            faq={CRYPTO_EXCHANGE_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/cryptocurrency-exchanges" />
          <ComparisonTable
            tools={featured as never[]}
            categories={CRYPTO_EXCHANGE_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/cryptocurrency-exchanges"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/cryptocurrency-exchanges" className="text-orange-400 hover:text-orange-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/cryptocurrency-exchanges" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = CRYPTO_EXCHANGE_CATEGORIES[cat];
                  const count = getCryptoExchangeCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/cryptocurrency-exchanges/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/cryptocurrency-exchanges" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="cryptocurrency-exchanges" />
      </div>
    </div>
  );
}
