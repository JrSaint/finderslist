...
export default async function AIToolsPage({ searchParams }: Props) {
  ...
  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering ? filterTools({ query: q, pricing, role }) : allTools;
  ...
  const itemListJsonLd = {
    ...
    "@type": "ItemList",
    name: "AI Tools",
    description: "Discover the best AI tools for writing, coding, image generation, video, productivity, and more. Browse 100+ curated AI tools with honest pros, cons, and pricing.",
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <FilterBar resultCount={filteredTools.length} />
          </Suspense>
        </section>
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ai-tools" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-semibold text-white">
                  {q ? `Results for "${q}"` : role ? `Tools for ${role.charAt(0).toUpperCase() + role.slice(1)}` : `${pricing?.charAt(0).toUpperCase()}${pricing?.slice(1)} Tools`}
                </h2>
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/ai-tools" className="text-violet-400 hover:text-violet-300 text-sm font-medium">
                  Clear all filters →
                </Link>
              </div>
            ) : (
