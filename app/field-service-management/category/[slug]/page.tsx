...
import type { FieldServiceCategory } from "@/lib/field-service-management";
...
export async function generateStaticParams() {
  return getAllFieldServiceCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = FIELD_SERVICE_CATEGORIES[slug as FieldServiceCategory];
  if (!category) return {};
  const tools = getFieldServiceToolsByCategory(slug as FieldServiceCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "field service management", category.label],
    alternates: { canonical: `https://www.finderslist.com/field-service-management/category/${slug}` },
  };
}

export default async function FieldServiceCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!FIELD_SERVICE_CATEGORIES[slug as FieldServiceCategory]) notFound();

  const category = FIELD_SERVICE_CATEGORIES[slug as FieldServiceCategory];
  const tools = getFieldServiceToolsByCategory(slug as FieldServiceCategory);
  const allCategories = getAllFieldServiceCategories();
  const totalTools = getAllFieldServiceTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/field-service-management/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/field-service-management/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/field-service-management" className="block w-full rounded-lg bg-orange-600 hover:bg-orange-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
