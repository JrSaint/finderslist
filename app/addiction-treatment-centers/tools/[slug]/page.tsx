...
export async function generateStaticParams() {
  return getAllAddictionTreatmentCenterTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getAddictionTreatmentCenterToolBySlug(slug);
  if (!tool) return {};
  const category = ADDICTION_TREATMENT_CATEGORIES[tool.category];
  return {
    title: `${tool.name} Review (2026) — Programs, Pros & Cons`,
    description: `${tool.tagline}. Honest review of ${tool.name}: strengths, considerations, treatment approach, and who it's best for.`,
    keywords: [tool.name, ...tool.tags, category.label],
    openGraph: {
      title: `${tool.name} Review 2026 | FindersList`,
      description: `${tool.tagline}. See honest pros, cons, treatment approach, and who it's best for.`,
    },
    alternates: { canonical: `https://www.finderslist.com/addiction-treatment-centers/tools/${tool.slug}` },
  };
}

export default async function AddictionTreatmentToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getAddictionTreatmentCenterToolBySlug(slug);
  if (!tool) notFound();

  const category = ADDICTION_TREATMENT_CATEGORIES[tool.category];
  const relatedTools = getAddictionTreatmentCenterToolsByCategory(tool.category).filter((t) => t.slug !== tool.slug).slice(0, 4);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Addiction Treatment Centers", item: "https://www.finderslist.com/addiction-treatment-centers" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/addiction-treatment-centers/category/${tool.category}` },
      { "@type": "ListItem", position: 4, name: tool.name },
    ],
  };

  const pricingDetail = {
    free: "Free resource — no cost to access information, helplines, or referrals.",
    freemium: "Free basic resources with optional paid treatment services.",
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href={`/addiction-treatment-centers/category/${tool.category}`} className="hover:text-slate-300 transition-colors">{category.label}</Link>
        <span>/</span>
        ...
        <div className="flex-1 min-w-0">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    <span className={`text-xs border rounded-full px-2.5 py-0.5 ${ADDICTION_TREATMENT_PRICING_COLORS[tool.pricing]}`}>{ADDICTION_TREATMENT_PRICING_LABELS[tool.pricing]}</span>
                  </div>
                  <p className="text-lg text-white/80 mb-3 leading-snug">{tool.tagline}</p>
                  <Link href={`/addiction-treatment-centers/category/${tool.category}`} className="inline-flex items-center gap-1.5 text-sm text-white/60 hover:text-white/90 transition-colors">
          ...
          {relatedTools.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTools.map((t) => (<ToolCard key={t.slug} tool={t as never} basePath="/addiction-treatment-centers" />))}
              </div>
            </section>
          )}
        </div>
        ...
        <aside className="lg:w-64 flex-shrink-0">
          <div className="sticky top-24 space-y-4">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <a href={visitUrl} target="_blank" rel="noopener noreferrer nofollow" className="block w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 px-4 py-2.5 text-sm font-medium text-white transition-colors text-center">Visit {tool.name} →</a>
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <Link href={`/addiction-treatment-centers/category/${tool.category}`} className="block w-full rounded-lg border border-white/10 hover:border-emerald-500/30 hover:bg-slate-800 px-4 py-2.5 text-sm text-slate-400 hover:text-white transition-all text-center">See all centers →</Link>
            </div>
          </div>
        </aside>
      </div>
