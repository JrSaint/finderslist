...
import type { HotelBookingCategory } from "@/lib/hotel-booking-platforms";
...
export async function generateStaticParams() {
  return getAllHotelBookingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = HOTEL_BOOKING_CATEGORIES[slug as HotelBookingCategory];
  if (!category) return {};
  const tools = getHotelBookingToolsByCategory(slug as HotelBookingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "hotel booking platforms", category.label],
    alternates: { canonical: `https://www.finderslist.com/hotel-booking-platforms/category/${slug}` },
  };
}

export default async function HotelBookingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!HOTEL_BOOKING_CATEGORIES[slug as HotelBookingCategory]) notFound();

  const category = HOTEL_BOOKING_CATEGORIES[slug as HotelBookingCategory];
  const tools = getHotelBookingToolsByCategory(slug as HotelBookingCategory);
  const allCategories = getAllHotelBookingCategories();
  const totalTools = getAllHotelBookingTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/hotel-booking-platforms/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/hotel-booking-platforms" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
