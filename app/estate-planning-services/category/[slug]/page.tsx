...
import type { EstatePlanningCategory } from "@/lib/estate-planning-services";
...
export async function generateStaticParams() {
  return getAllEstatePlanningCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ESTATE_PLANNING_CATEGORIES[slug as EstatePlanningCategory];
  if (!category) return {};
  const tools = getEstatePlanningToolsByCategory(slug as EstatePlanningCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "estate planning services", category.label],
    alternates: { canonical: `https://www.finderslist.com/estate-planning-services/category/${slug}` },
  };
}

export default async function EstatePlanningCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ESTATE_PLANNING_CATEGORIES[slug as EstatePlanningCategory]) notFound();

  const category = ESTATE_PLANNING_CATEGORIES[slug as EstatePlanningCategory];
  const tools = getEstatePlanningToolsByCategory(slug as EstatePlanningCategory);
  const allCategories = getAllEstatePlanningCategories();
  const totalTools = getAllEstatePlanningTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/estate-planning-services/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/estate-planning-services" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
