...
import {
  getFeaturedEstatePlanningTools,
  getAllEstatePlanningTools,
  getAllEstatePlanningCategories,
  ESTATE_PLANNING_CATEGORIES,
  getEstatePlanningCategoryCount,
  filterEstatePlanningTools,
} from "@/lib/estate-planning-services";
import { ESTATE_PLANNING_EDITORIAL } from "@/data/estate-planning-services";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/estate-planning-services" } };
  }
  return {
    title: "Best Online Estate Planning Services (2026) — 15+ Options Compared",
    description: "Compare the best online estate planning services for wills, trusts, and power of attorney. Reviews of Trust & Will, LegalZoom, Nolo, and more.",
    keywords: ["estate planning services","online will services 2026","living trust","estate planning","online will maker"],
    alternates: { canonical: "https://www.finderslist.com/estate-planning-services" },
  };
}

export default async function EstatePlanningPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedEstatePlanningTools();
  const allTools = getAllEstatePlanningTools();
  const categories = getAllEstatePlanningCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterEstatePlanningTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Online Estate Planning Services", item: "https://www.finderslist.com/estate-planning-services" },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Online Estate Planning Services",
    description: "Compare the best online estate planning services for wills, trusts, and power of attorney. Reviews of Trust & Will, LegalZoom, Nolo, and more.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/estate-planning-services/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ESTATE_PLANNING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated estate planning services
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of estate planning services. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/estate-planning-services" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <section className="mt-10 p-5 rounded-2xl border border-white/8 bg-slate-900/30">
          <Suspense fallback={null}>
            <GenericFilterBar
              roles={{}}
              basePath="/estate-planning-services"
        ...
        {!isFiltering && (
          <EditorialSection
            title={ESTATE_PLANNING_EDITORIAL.title}
            intro={ESTATE_PLANNING_EDITORIAL.intro}
            buyerGuide={ESTATE_PLANNING_EDITORIAL.buyerGuide}
            faq={ESTATE_PLANNING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/estate-planning-services" />
          <ComparisonTable
            tools={featured as never[]}
            categories={ESTATE_PLANNING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/estate-planning-services"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/estate-planning-services" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/estate-planning-services" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ESTATE_PLANNING_CATEGORIES[cat];
                  const count = getEstatePlanningCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/estate-planning-services/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/estate-planning-services" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="estate-planning-services" />
      </div>
    </div>
  );
}
