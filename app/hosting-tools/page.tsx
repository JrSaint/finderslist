...
import {
  getFeaturedHostingTools,
  getAllHostingTools,
  getAllHostingCategories,
  HOSTING_CATEGORIES,
  getHostingCategoryCount,
  filterHostingTools,
  HOSTING_ROLES,
} from "@/lib/hosting-tools";
import { HOSTING_EDITORIAL } from "@/data/hosting-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/hosting-tools" } };
  }
  return {
    title: "Best Cloud & Web Hosting Directory (2026) — 30+ Tools Reviewed",
    description: "The most comprehensive directory of cloud and web hosting platforms. Compare Kinsta, WP Engine, Cloudflare, AWS, SiteGround, and 30+ more with honest reviews.",
    alternates: { canonical: "https://www.finderslist.com/hosting-tools" },
  };
}

const roles = Object.fromEntries(
  Object.entries(HOSTING_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function HostingToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedHostingTools();
  const allTools = getAllHostingTools();
  const categories = getAllHostingCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterHostingTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Cloud & Web Hosting", item: "https://www.finderslist.com/hosting-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/hosting-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (HOSTING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={HOSTING_EDITORIAL.title}
            intro={HOSTING_EDITORIAL.intro}
            buyerGuide={HOSTING_EDITORIAL.buyerGuide}
            faq={HOSTING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/hosting-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={HOSTING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/hosting-tools"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/hosting-tools" className="text-sky-400 hover:text-sky-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/hosting-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = HOSTING_CATEGORIES[cat];
                  const count = getHostingCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/hosting-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-sky-500/30 transition-all group">
            ...
            <section className="mt-14">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {featured.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/hosting-tools" />))}
              </div>
            </section>
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/hosting-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="hosting-tools" />
      </div>
    </div>
  );
}
