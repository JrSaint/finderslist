...
import type { SeoToolCategory } from "@/lib/seo-tools";
...
export async function generateStaticParams() {
  return getAllSeoToolCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = SEO_TOOL_CATEGORIES[slug as SeoToolCategory];
  if (!category) return {};
  const tools = getSeoToolToolsByCategory(slug as SeoToolCategory);
  return {
    ...
    keywords: [`${category.label}`, `best ${category.label} 2026`, "SEO tools", category.label],
    alternates: { canonical: `https://www.finderslist.com/seo-tools/category/${slug}` },
  };
}

export default async function SeoToolCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!SEO_TOOL_CATEGORIES[slug as SeoToolCategory]) notFound();

  const category = SEO_TOOL_CATEGORIES[slug as SeoToolCategory];
  const tools = getSeoToolToolsByCategory(slug as SeoToolCategory);
  const allCategories = getAllSeoToolCategories();
  const totalTools = getAllSeoToolTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/seo-tools/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/seo-tools" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
