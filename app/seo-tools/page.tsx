...
import {
  getFeaturedSeoToolTools,
  getAllSeoToolTools,
  getAllSeoToolCategories,
  SEO_TOOL_CATEGORIES,
  getSeoToolCategoryCount,
  filterSeoToolTools,
} from "@/lib/seo-tools";
import { SEO_TOOL_EDITORIAL } from "@/data/seo-tools";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/seo-tools" } };
  }
  return {
    title: "Best SEO Tools (2026) — Keyword, Rank, Backlink & Technical SEO",
    description: "Compare the best SEO tools in 2026. Reviews of Ahrefs, Semrush, Moz, Screaming Frog, Surfer SEO, and more for keyword research, rank tracking, and audits.",
    keywords: ["seo tools","best seo tools 2026","keyword research tools","rank tracking software","backlink analysis"],
    alternates: { canonical: "https://www.finderslist.com/seo-tools" },
  };
}

export default async function SeoToolPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedSeoToolTools();
  const allTools = getAllSeoToolTools();
  const categories = getAllSeoToolCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterSeoToolTools({ query: q, pricing, role })
    : allTools;
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/seo-tools/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (SEO_TOOL_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-sm text-amber-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated SEO tools
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of SEO tools. Compare top options with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/seo-tools" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={SEO_TOOL_EDITORIAL.title}
            intro={SEO_TOOL_EDITORIAL.intro}
            buyerGuide={SEO_TOOL_EDITORIAL.buyerGuide}
            faq={SEO_TOOL_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/seo-tools" />
          <ComparisonTable
            tools={featured as never[]}
            categories={SEO_TOOL_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/seo-tools"
          />
          </>
        )}
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/seo-tools" className="text-amber-400 hover:text-amber-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/seo-tools" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = SEO_TOOL_CATEGORIES[cat];
                  const count = getSeoToolCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/seo-tools/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/seo-tools" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="seo-tools" />
      </div>
    </div>
  );
}
