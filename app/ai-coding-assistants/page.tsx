...
import { AI_CODING_EDITORIAL } from "@/data/ai-coding-assistants";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/ai-coding-assistants" } };
  }
  return {
    ...
    alternates: { canonical: "https://www.finderslist.com/ai-coding-assistants" },
  };
}
...
export default async function AICodingPage({ searchParams }: Props) {
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (AI_CODING_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/ai-coding-assistants" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <EditorialSection
            title={AI_CODING_EDITORIAL.title}
            intro={AI_CODING_EDITORIAL.intro}
            buyerGuide={AI_CODING_EDITORIAL.buyerGuide}
            faq={AI_CODING_EDITORIAL.faq}
          />
        )}

        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ai-coding-assistants" />
          <ComparisonTable
            tools={featured as never[]}
            categories={AI_CODING_CATEGORIES as Record<string, { label: string; emoji: string }>}
            basePath="/ai-coding-assistants"
          />
          </>
        )}
        ...
        <RelatedDirectories currentSlug="ai-coding-assistants" />
      </div>
    </div>
  );
}
