...
import type { AICodingCategory } from "@/lib/ai-coding-assistants";
...
export async function generateStaticParams() {
  return getAllAICodingCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = AI_CODING_CATEGORIES[slug as AICodingCategory];
  if (!category) return {};
  const tools = getAICodingToolsByCategory(slug as AICodingCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "ai coding assistants", category.label],
    alternates: { canonical: `https://www.finderslist.com/ai-coding-assistants/category/${slug}` },
  };
}

export default async function AICodingCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!AI_CODING_CATEGORIES[slug as AICodingCategory]) notFound();

  const category = AI_CODING_CATEGORIES[slug as AICodingCategory];
  const tools = getAICodingToolsByCategory(slug as AICodingCategory);
  const allCategories = getAllAICodingCategories();
  const totalTools = getAllAICodingTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/ai-coding-assistants/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/ai-coding-assistants/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/ai-coding-assistants" className="block w-full rounded-lg bg-green-600 hover:bg-green-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
