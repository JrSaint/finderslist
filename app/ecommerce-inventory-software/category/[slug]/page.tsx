...
import type { EcommerceInventoryCategory } from "@/lib/ecommerce-inventory-software";
...
export async function generateStaticParams() {
  return getAllEcommerceInventoryCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = ECOMMERCE_INVENTORY_CATEGORIES[slug as EcommerceInventoryCategory];
  if (!category) return {};
  const tools = getEcommerceInventoryToolsByCategory(slug as EcommerceInventoryCategory);
  return {
    ...
    keywords: [`${category.label} tools`, `best ${category.label} tools 2026`, "e-commerce inventory software", category.label],
    alternates: { canonical: `https://www.finderslist.com/ecommerce-inventory-software/category/${slug}` },
  };
}

export default async function EcommerceInventoryCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!ECOMMERCE_INVENTORY_CATEGORIES[slug as EcommerceInventoryCategory]) notFound();

  const category = ECOMMERCE_INVENTORY_CATEGORIES[slug as EcommerceInventoryCategory];
  const tools = getEcommerceInventoryToolsByCategory(slug as EcommerceInventoryCategory);
  const allCategories = getAllEcommerceInventoryCategories();
  const totalTools = getAllEcommerceInventoryTools().length;
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/ecommerce-inventory-software/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/ecommerce-inventory-software" className="block w-full rounded-lg bg-teal-600 hover:bg-teal-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
