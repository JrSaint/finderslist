...
import {
  getFeaturedEcommerceInventoryTools,
  getAllEcommerceInventoryTools,
  getAllEcommerceInventoryCategories,
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  if (isFiltering) {
    return { robots: { index: false, follow: false }, alternates: { canonical: "https://www.finderslist.com/ecommerce-inventory-software" } };
  }
  return {
    title: "Best E-Commerce Inventory Management (2026) — 20+ Tools Reviewed",
    description: "Compare the best e-commerce inventory management software. Reviews of Cin7, Linnworks, Fishbowl, and more.",
    keywords: ["ecommerce inventory management","inventory software 2026","multi-channel inventory","warehouse management","order management system"],
    alternates: { canonical: "https://www.finderslist.com/ecommerce-inventory-software" },
  };
}

export default async function EcommerceInventoryPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedEcommerceInventoryTools();
  const allTools = getAllEcommerceInventoryTools();
  const categories = getAllEcommerceInventoryCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterEcommerceInventoryTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "E-Commerce Inventory Management", item: "https://www.finderslist.com/ecommerce-inventory-software" },
    ],
  };
  ...
  const itemListJsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "E-Commerce Inventory Management",
    description: "Compare the best e-commerce inventory management software. Reviews of Cin7, Linnworks, Fishbowl, and more.",
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/ecommerce-inventory-software/tools/${tool.slug}`,
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: (ECOMMERCE_INVENTORY_EDITORIAL.faq || []).map((f) => ({
      "@type": "Question",
      name: f.question,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <div className="relative mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-20 text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-teal-500/30 bg-teal-500/10 px-4 py-1.5 text-sm text-teal-300 backdrop-blur-sm">
            ...
            {allTools.length}+ curated e-commerce inventory software
          </div>
          ...
          <p className="text-lg text-slate-400 max-w-2xl mx-auto mb-8 leading-relaxed">
            The most comprehensive directory of e-commerce inventory software. Compare top tools with honest reviews, pricing, and real-world use cases.
          </p>
          <div className="max-w-2xl mx-auto mb-8">
            <SearchBar large basePath="/ecommerce-inventory-software" tools={allTools as never[]} />
          </div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!isFiltering && (
          <>
          <CategoryAtAGlance tools={allTools as never[]} basePath="/ecommerce-inventory-software" />
          <ComparisonTable
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/ecommerce-inventory-software" className="text-teal-400 hover:text-teal-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} basePath="/ecommerce-inventory-software" />
                ))}
              </div>
            )}
          </section>
          ...
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = ECOMMERCE_INVENTORY_CATEGORIES[cat];
                  const count = getEcommerceInventoryCategoryCount(cat);
                  return (
                    <Link
                      key={cat}
                      href={`/ecommerce-inventory-software/category/${cat}`}
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (
                  <ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/ecommerce-inventory-software" />
                ))}
              </div>
            </section>
          </>
        ...
        <RelatedDirectories currentSlug="ecommerce-inventory-software" />
      </div>
    </div>
  );
}
