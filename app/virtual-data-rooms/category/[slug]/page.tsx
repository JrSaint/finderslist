...
import type { VirtualDataRoomCategory } from "@/lib/virtual-data-rooms";
...
export async function generateStaticParams() {
  return getAllVirtualDataRoomCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = VIRTUAL_DATA_ROOM_CATEGORIES[slug as VirtualDataRoomCategory];
  if (!category) return {};
...
export default async function VirtualDataRoomCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!VIRTUAL_DATA_ROOM_CATEGORIES[slug as VirtualDataRoomCategory]) notFound();

  const category = VIRTUAL_DATA_ROOM_CATEGORIES[slug as VirtualDataRoomCategory];
  const tools = getVirtualDataRoomToolsByCategory(slug as VirtualDataRoomCategory);
  const allCategories = getAllVirtualDataRoomCategories();
  const totalTools = getAllVirtualDataRoomTools().length;

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: `Best ${category.label} Virtual Data Rooms`,
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-6 text-sm text-slate-500">Showing {tools.length} platform{tools.length !== 1 ? "s" : ""}</div>
          <CategoryGuide guide={(category as { guide?: string }).guide} />
          {tools.length === 0 ? (
            <div className="text-center py-16 text-slate-500">No platforms in this category yet.</div>
          ) : (
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/virtual-data-rooms/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              <p className="text-sm text-slate-400 mb-3">Know a great platform?</p>
              <Link href="/virtual-data-rooms" className="block w-full rounded-lg bg-blue-600 hover:bg-blue-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
              <p className="text-xs text-slate-600 mt-2">{totalTools} platforms listed</p>
            </div>
          </div>
        </aside>
      </div>
