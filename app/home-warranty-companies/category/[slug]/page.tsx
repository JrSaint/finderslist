...
import type { HomeWarrantyCategory } from "@/lib/home-warranty-companies";
...
export async function generateStaticParams() {
  return getAllHomeWarrantyCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = HOME_WARRANTY_CATEGORIES[slug as HomeWarrantyCategory];
  if (!category) return {};
  ...
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/home-warranty-companies/category/${slug}` },
  };
}

export default async function HomeWarrantyCategoryPage({ params }: Props) {
  const { slug } = await params;

  if (!HOME_WARRANTY_CATEGORIES[slug as HomeWarrantyCategory]) notFound();

  const category = HOME_WARRANTY_CATEGORIES[slug as HomeWarrantyCategory];
  const tools = getHomeWarrantyToolsByCategory(slug as HomeWarrantyCategory);
  const allCategories = getAllHomeWarrantyCategories();
  const totalTools = getAllHomeWarrantyTools().length;
  ...
  const jsonLd = {
    ...
    itemListElement: tools.map((tool, i) => ({
      ...
      url: `https://www.finderslist.com/home-warranty-companies/tools/${tool.slug}`,
    })),
  };
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/home-warranty-companies/category/${cat}`}
            ...
            <div className="mt-6 rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/home-warranty-companies" className="block w-full rounded-lg bg-amber-600 hover:bg-amber-500 px-4 py-2 text-sm font-medium text-white transition-colors text-center">Browse All</Link>
