...
import {
  getFeaturedNoCodeTools,
  getAllNoCodeTools,
  getAllNoCodeCategories,
  NO_CODE_CATEGORIES,
  getNoCodeCategoryCount,
  filterNoCodeTools,
  NO_CODE_ROLES,
...
const roles = Object.fromEntries(
  Object.entries(NO_CODE_ROLES).map(([k, v]) => [k, { emoji: v.emoji, label: v.label }])
);

export default async function NoCodeToolsPage({ searchParams }: Props) {
  const { q, pricing, role } = await searchParams;
  const featured = getFeaturedNoCodeTools();
  const allTools = getAllNoCodeTools();
  const categories = getAllNoCodeCategories();

  const isFiltering = !!(q || pricing || role);
  const filteredTools = isFiltering
    ? filterNoCodeTools({ query: q, pricing, role })
    : allTools;
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "No-Code & Low-Code Tools", item: "https://www.finderslist.com/no-code-tools" },
    ],
  };
  ...
  const itemListJsonLd = {
    ...
    itemListElement: allTools.slice(0, 10).map((tool, i) => ({
      ...
      position: i + 1,
      name: tool.name,
      url: `https://www.finderslist.com/no-code-tools/tools/${tool.slug}`,
          ...
          <section className="mt-10 mb-16">
            ...
            {filteredTools.length === 0 ? (
              <div className="text-center py-20 text-slate-500">
                ...
                <Link href="/no-code-tools" className="text-yellow-400 hover:text-yellow-300 text-sm font-medium">Clear all filters →</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} basePath="/no-code-tools" />))}
              </div>
            )}
          </section>
        ) : (
          <>
            <section className="mt-14">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {categories.map((cat) => {
                  const info = NO_CODE_CATEGORIES[cat];
                  const count = getNoCodeCategoryCount(cat);
                  return (
                    <Link key={cat} href={`/no-code-tools/category/${cat}`} className="flex flex-col gap-1.5 p-4 rounded-xl border border-white/8 bg-slate-900/40 hover:bg-slate-800/60 hover:border-yellow-500/30 transition-all group">
            ...
            <section className="mt-12 mb-16">
              ...
              <div className="flex flex-col gap-2">
                {allTools.map((tool) => (<ToolCard key={tool.slug} tool={tool as never} variant="compact" basePath="/no-code-tools" />))}
              </div>
            </section>
          </>
        )}

        <RelatedDirectories currentSlug="no-code-tools" />
      </div>
    </div>
  );
}
