...
import { getNoCodeToolBySlug, getAllNoCodeTools, getNoCodeToolsByCategory, NO_CODE_CATEGORIES, NO_CODE_PRICING_LABELS, NO_CODE_PRICING_COLORS, NO_CODE_ROLES, type NoCodeRoleKey } from "@/lib/no-code-tools";
...
export async function generateStaticParams() {
  return getAllNoCodeTools().map((tool) => ({ slug: tool.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const tool = getNoCodeToolBySlug(slug);
  if (!tool) return {};
  const category = NO_CODE_CATEGORIES[tool.category];
  return {
    ...
    alternates: { canonical: `https://www.finderslist.com/no-code-tools/tools/${tool.slug}` },
  };
}

function getBestForRoles(tool: ReturnType<typeof getNoCodeToolBySlug>) {
  if (!tool) return [];
  return (Object.entries(NO_CODE_ROLES) as [NoCodeRoleKey, typeof NO_CODE_ROLES[NoCodeRoleKey]][])
    .filter(([, config]) =>
...
export default async function NoCodeToolPage({ params }: Props) {
  const { slug } = await params;
  const tool = getNoCodeToolBySlug(slug);
  if (!tool) notFound();

  const category = NO_CODE_CATEGORIES[tool.category];
  const relatedTools = getNoCodeToolsByCategory(tool.category).filter((t) => t.slug !== tool.slug).slice(0, 4);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "NoCode Tools", item: "https://www.finderslist.com/no-code-tools" },
      { "@type": "ListItem", position: 3, name: category.label, item: `https://www.finderslist.com/no-code-tools/category/${tool.category}` },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-6 flex-wrap">
        ...
        <span>/</span>
        <Link href="/no-code-tools" className="hover:text-slate-300 transition-colors">NoCode Tools</Link>
        <span>/</span>
        <Link href={`/no-code-tools/category/${tool.category}`} className="hover:text-slate-300 transition-colors">{category.label}</Link>
        <span>/</span>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 min-w-0">
              ...
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-1.5">
                    ...
                    {tool.featured && <span className="text-xs bg-yellow-500/30 text-yellow-200 border border-yellow-400/30 rounded-full px-2.5 py-0.5">Featured</span>}
                    <span className={`text-xs border rounded-full px-2.5 py-0.5 ${NO_CODE_PRICING_COLORS[tool.pricing]}`}>{NO_CODE_PRICING_LABELS[tool.pricing]}</span>
                  </div>
                  <p className="text-lg text-white/80 mb-3 leading-snug">{tool.tagline}</p>
                  <Link href={`/no-code-tools/category/${tool.category}`} className="inline-flex items-center gap-1.5 text-sm text-white/60 hover:text-white/90 transition-colors">
          ...
          <div className="sm:hidden mb-6">
            <a href={isDead ? tool.url : visitUrl} target="_blank" rel={tool.affiliateUrl ? "sponsored nofollow noopener noreferrer" : "noopener noreferrer"} aria-disabled={isDead || undefined} className={`flex items-center justify-center gap-2 rounded-xl bg-yellow-600 hover:bg-yellow-500 px-6 py-3 font-semibold text-white transition-colors${isDead ? " opacity-50 grayscale pointer-events-none" : ""}`}>
              {isDead ? "No longer available" : <>Visit {tool.name}</>}
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
            </a>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              ...
              {tool.cons && tool.cons.length > 0 && (
                <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-5">
                  ...
                  <ul className="space-y-2">
                    {tool.cons.map((con, i) => (<li key={i} className="flex items-start gap-2 text-sm text-slate-300"><span className="text-yellow-500 mt-0.5 flex-shrink-0">!</span>{con}</li>))}
                  </ul>
                </div>
              )}
            </div>
              ...
              <div className="space-y-3">
                {tool.useCases.map((useCase, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full bg-yellow-500/20 border border-yellow-500/30 flex items-center justify-center flex-shrink-0 text-xs font-bold text-yellow-400 mt-0.5">{i + 1}</div>
            ...
            <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-6 mb-5">
              ...
              <div className="flex flex-wrap gap-2">
                {bestForRoles.map((role) => (
                  <Link key={role.key} href={`/no-code-tools?role=${role.key}`} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-white/10 bg-slate-800/60 hover:border-yellow-500/30 hover:bg-slate-700/60 text-sm text-slate-300 hover:text-white transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <p className="text-sm font-semibold text-white mb-1">{NO_CODE_PRICING_LABELS[tool.pricing]}</p>
          ...
          {relatedTools.length > 0 && (
            <section>
              <div className="flex items-center justify-between mb-4">
                ...
                <Link href={`/no-code-tools/category/${tool.category}`} className="text-xs text-yellow-400 hover:text-yellow-300 transition-colors">See all →</Link>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {relatedTools.map((t) => (<ToolCard key={t.slug} tool={t as never} basePath="/no-code-tools" />))}
              </div>
            </section>
          )}
        </div>
        ...
        <aside className="lg:w-64 flex-shrink-0">
          <div className="sticky top-24 space-y-4">
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <a href={isDead ? tool.url : visitUrl} target="_blank" rel={tool.affiliateUrl ? "sponsored nofollow noopener noreferrer" : "noopener noreferrer"} aria-disabled={isDead || undefined} className={`block w-full rounded-lg bg-yellow-600 hover:bg-yellow-500 px-4 py-2.5 text-sm font-medium text-white transition-colors text-center${isDead ? " opacity-50 grayscale pointer-events-none" : ""}`}>{isDead ? "No longer available" : <>Open {tool.name} →</>}</a>
              <p className="text-xs text-slate-600 mt-2 text-center">{NO_CODE_PRICING_LABELS[tool.pricing]} · {category.label}</p>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              <p className="text-sm font-medium text-white mb-2">💰 Pricing</p>
              <span className={`inline-flex text-xs border rounded-full px-2.5 py-1 mb-2 ${NO_CODE_PRICING_COLORS[tool.pricing]}`}>{NO_CODE_PRICING_LABELS[tool.pricing]}</span>
              <p className="text-xs text-slate-500 leading-relaxed">{isDead ? "" : tool.startingPrice ? `From ${tool.startingPrice}. ` : ""}{pricingText}</p>
            </div>
            {tool.pros && tool.pros.length > 0 && (
              <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
            ...
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4">
              ...
              <Link href={`/no-code-tools/category/${tool.category}`} className="block w-full rounded-lg border border-white/10 hover:border-yellow-500/30 hover:bg-slate-800 px-4 py-2.5 text-sm text-slate-400 hover:text-white transition-all text-center">See all tools →</Link>
            </div>
            <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4 text-center">
              ...
              <Link href="/no-code-tools/submit" className="block w-full rounded-lg bg-slate-800 hover:bg-slate-700 border border-white/10 px-4 py-2 text-sm font-medium text-slate-300 transition-colors text-center">Submit a Tool</Link>
            </div>
          </div>
        </aside>
      </div>
