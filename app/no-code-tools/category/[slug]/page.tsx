...
import { getNoCodeToolsByCategory, NO_CODE_CATEGORIES, getAllNoCodeCategories, getAllNoCodeTools } from "@/lib/no-code-tools";
import type { NoCodeCategory } from "@/lib/no-code-tools";
...
export async function generateStaticParams() {
  return getAllNoCodeCategories().map((cat) => ({ slug: cat }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const category = NO_CODE_CATEGORIES[slug as NoCodeCategory];
  if (!category) return {};
  const tools = getNoCodeToolsByCategory(slug as NoCodeCategory);
  return {
...
export default async function NoCodeCategoryPage({ params }: Props) {
  const { slug } = await params;
  if (!NO_CODE_CATEGORIES[slug as NoCodeCategory]) notFound();
  const category = NO_CODE_CATEGORIES[slug as NoCodeCategory];
  const tools = getNoCodeToolsByCategory(slug as NoCodeCategory);
  const allCategories = getAllNoCodeCategories();
  const totalTools = getAllNoCodeTools().length;
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-4 mb-6">
            <div className="w-14 h-14 rounded-2xl bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center text-3xl">{category.emoji}</div>
            <div>
          ...
          {tools.length === 0 ? (
            <div className="text-center py-16 text-slate-500">No tools in this category yet. <Link href="/no-code-tools/submit" className="text-yellow-400 hover:text-yellow-300">Submit one!</Link></div>
          ) : (
          ...
          <div className="sticky top-24">
            ...
            <div className="space-y-1">
              {allCategories.map((cat) => {
                ...
                return (
                  <Link key={cat} href={`/no-code-tools/category/${cat}`} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${isActive ? "bg-yellow-500/20 text-yellow-300 border border-yellow-500/30" : "text-slate-400 hover:text-white hover:bg-white/5"}`}>
